# B1 Grammatik-Leitfaden: Die Schwelle zur Autonomie (The Bridge to Independence)

---
🧭 **CEFR Stufen-Navigation:**  
[A1 - Anfänger](../A1/Grammatik.md) ➔ [A2 - Grundstufe](../A2/Grammatik.md) ➔ **[B1 - Mittelstufe](./Grammatik.md)** ➔ [B2 - Selbstständige Sprachverwendung](../B2/Grammatik.md) ➔ [C1 - Fachkundig](../C1/Grammatik.md) ➔ [C2 - Meisterschaft](../C2/Grammatik.md)
---

> **Willkommen auf Stufe B1!**  
> Das B1-Niveau (Zertifikat Deutsch / Goethe-Zertifikat B1) ist der entscheidende Wendepunkt in deiner Sprachbiografie. Du sprichst nicht mehr nur über einfache Abläufe, sondern äußerst fundierte Meinungen, formulierst Hypothesen, beschreibst Prozesse objektiv im Passiv und verknüpfst komplexe Gedankengänge zu fließenden Texten. Hier wird dein Deutsch erwachsen.

---

## 1. Wie du die Dateien in diesem Ordner grammatikalisch nutzt

Nutze die B1-Dateien als integriertes System zur Konstruktion mehrteiliger Argumente und präziser Satzgefüge:

```
                      ┌──────────────────────────────┐
                      │       Grammatik.md           │  <-- Relativsätze, Passiv, Konjunktiv II, TeKaMoLo
                      └──────────────┬───────────────┘
                                     │
         ┌───────────────────────────┼───────────────────────────┐
         ▼                           ▼                           ▼
 ┌───────────────┐           ┌───────────────┐           ┌───────────────┐
 │   Verbs.md    │           │   Cases.md    │           │Prepositions.md│
 │ (Passiv,      │           │ (Genitiv,     │           │ (Genitiv- &   │
 │  Konjunktiv II│           │  N-Deklination│           │  feste Verb-  │
 │  & Infinitiv) │           │  Objektregeln)│           │  präpositionen│
 └───────┬───────┘           └───────┬───────┘           └───────┬───────┘
         │                           │                           │
         └───────────────────────────┼───────────────────────────┘
                                     ▼
                      ┌──────────────────────────────┐
                      │ Pronouns / Adjectives /      │
                      │ Vocabulary.md                │
                      │ (Relativpronomen, da-/wo-,   │
                      │  Doppelkonnektoren, Diskurs) │
                      └──────────────────────────────┘
```

### Der B1-Produktionszirkel:
1. **Den Modus & die Perspektive wählen ([`Verbs.md`](file:///e:/Projects/VS_Projects/DeutscheLernen/B1/Verbs.md)):**  
   Entscheide über das Register:
   - Geht es um eine Tatsache (*Indikativ*), eine höfliche Bitte/Hypothese (*Konjunktiv II: würde + Infinitiv / wäre / hätte / könnte*) oder einen objektiven Vorgang (*Passiv: werden + Partizip II*)?
2. **Den Genitiv & Präpositionen meisterhaft setzen ([`Cases.md`](file:///e:/Projects/VS_Projects/DeutscheLernen/B1/Cases.md) & [`Prepositions.md`](file:///e:/Projects/VS_Projects/DeutscheLernen/B1/Prepositions.md)):**  
   Ersetze umgangssprachliche Umschreibungen mit *von* durch den echten Genitiv (*das Auto meines Vaters*, *während des Urlaubs*, *trotz des schlechten Wetters*).
3. **Pronominaladverbien & Feste Verben verbinden ([`Pronouns.md`](file:///e:/Projects/VS_Projects/DeutscheLernen/B1/Pronouns.md)):**  
   Kopple Verben mit festen Präpositionen: Handelt es sich um eine Sache (*worüber? -> darüber*) oder eine Person (*über wen? -> über ihn*)?
4. **Relativsätze zur Verdichtung einsetzen ([`Pronouns.md`](file:///e:/Projects/VS_Projects/DeutscheLernen/B1/Pronouns.md)):**  
   Statt zweier abgehackter Sätze baust du Relativsätze ein (*Das ist der Kollege, **mit dem** ich gestern gesprochen habe*).
5. **Nuancen & Konnektoren einweben ([`Adjectives.md`](file:///e:/Projects/VS_Projects/DeutscheLernen/B1/Adjectives.md) & [`Vocabulary.md`](file:///e:/Projects/VS_Projects/DeutscheLernen/B1/Vocabulary.md)):**  
   Nutze Doppelkonnektoren (*nicht nur... sondern auch*, *sowohl... als auch*) für stilistisch souveräne Übergänge.

---

## 2. Der grammatikalische B1-Kanon (Die Meilensteine)

| Thema | Kernkompetenz | Musterbeispiel |
| :--- | :--- | :--- |
| **Passiv (Vorgangspassiv)** | Das Geschehen steht im Mittelpunkt, nicht der Handelnde | *Das Haus **wird renoviert**.* / *Der Brief **wurde gestern abgeschickt**.* |
| **Konjunktiv II (Gegenwart)** | Höflichkeit, Wünsche, Ratschläge, irreale Bedingungen | *Wenn ich mehr Zeit **hätte**, **würde** ich eine Reise **machen**.* |
| **Der Genitiv (Wessen-Fall)** | Besitz & Zugehörigkeit; maskulin/neutral: `-(e)s` | *Die Meinung **der Lehrerin** / das Auto **des Vaters**.* |
| **Genitiv-Präpositionen** | *während, wegen, trotz, (an)statt* | ***Trotz des Regens** gingen wir spazieren.* |
| **Relativsätze** | Nebensätze zur näheren Beschreibung eines Nomens | *Der Mann, **den** du dort siehst, ist mein Chef.* |
| **Infinitiv mit *zu*** | Nach bestimmten Verben, Nomen oder Adjektiven | *Ich habe vor, dieses Jahr eine Prüfung **abzulegen**.* |
| **Finalsätze (*um... zu* vs. *damit*)** | Zweck/Absicht (*um... zu* bei Subjektgleichheit) | *Ich lerne Deutsch, **um** in Deutschland **zu studieren**.* |
| **Doppelkonnektoren** | Zweiteilige Satzverbindungen | *Er spricht **sowohl** Deutsch **als auch** Französisch.* |
| **Pronominaladverbien** | *da(r)-* und *wo(r)-* Verbindungen | ***Worauf** wartest du? Ich warte **darauf**, dass...* |
| **N-Deklination** | Schwache Maskulina erhalten im Akk./Dat./Gen. ein `-(e)n` | *Ich spreche mit dem Kolleg**en** / dem Kunde**n**.* |
| **Temporale Konjunktionen** | *als* (einmalig Vergangenheit) vs. *wenn* (wiederholt/Gegenwart) | ***Als** ich ein Kind war...* / ***Wenn** es regnet...* |

---

## 3. Satzstruktur & Wortstellung (Syntax auf Stufe B1)

### A. TeKaMoLo: Die Reihenfolge der Angaben im Mittelfeld
Wenn im Satz mehrere Adverbialangaben zusammentreffen, ordnet die deutsche Hochsprache diese nach dem **TeKaMoLo-Prinzip**:

```
  Temporaler Schritt      Kausaler Schritt       Modaler Schritt       Lokaler Schritt
      (WANN?)                 (WARUM?)               (WIE?)                (WO / WOHIN?)
┌───────────────────────┬──────────────────────┬─────────────────────┬─────────────────────┐
│ Temporal              │ Kausal               │ Modal               │ Lokal               │
│ gestern               │ wegen des Staus      │ mit dem Fahrrad     │ ins Büro            │
└───────────────────────┴──────────────────────┴─────────────────────┴─────────────────────┘
```
- **Beispielsatz:**  
  *Ich bin **gestern** (Temporal) **wegen des Streiks** (Kausal) **mit dem Taxi** (Modal) **zum Flughafen** (Lokal) gefahren.*

---

### B. Die Pronomen-Sonderregel im Mittelfeld

Vergleiche diese beiden Sätze sehr genau:

1. **Beide Objekte sind Nomen:** 👉 **Dativ vor Akkusativ!**  
   *Ich gebe **meinem Bruder** (Dativ / Nomen) **den Schlüssel** (Akkusativ / Nomen).*
2. **Beide Objekte sind Pronomen:** 👉 **Akkusativ vor Dativ!**  
   *Ich gebe **ihn** (Akkusativ / Pronomen) **ihm** (Dativ / Pronomen).*

> **Lehrer-Merksatz:** *Pronomen drängeln sich immer ganz nach vorne und das Akkusativ-Pronomen überholt das Dativ-Pronomen!*

---

### C. Relativsätze (Das Schweizer Taschenmesser des B1-Sprechers)

Ein Relativsatz richtet sich nach zwei Faktoren:
1. **Genus und Numerus** (maskulin, feminin, neutral, Plural) kommen vom **Bezugswort** im Hauptsatz.
2. **Der Kasus** kommt von der **Rolle im Relativsatz** (wer tut was im Nebensatz?).

```
  Hauptsatz mit Bezugswort       Komma     Relativpronomen (Kasus aus NS)   Mittelfeld     Verb am Ende
┌──────────────────────────────┬─────────┬────────────────────────────────┬──────────────┬─────────────┐
│ Das ist der Kollege (mask.), │ ,       │ **der** (Subjekt = Nom.)       │ gestern neu  │ **kam**.    │
│ Das ist der Kollege (mask.), │ ,       │ **den** (Akk.-Objekt = Akk.)   │ ich gestern  │ **sah**.    │
│ Das ist der Kollege (mask.), │ ,       │ **dem** (Dat.-Objekt = Dat.)   │ ich geholfen │ **habe**.   │
│ Das ist der Kollege (mask.), │ ,       │ **dessen** (Besitz = Gen.)     │ Auto kaputt  │ **ist**.    │
└──────────────────────────────┴─────────┴────────────────────────────────┴──────────────┴─────────────┘
```

---

### D. Infinitivsätze mit *zu* & Trennbare Verben

Steht im Infinitivsatz ein trennbares Verb, schiebt sich das `-zu-` **zwischen** Vorsilbe und Verbstamm:
- *anfangen* -> Es ist wichtig, jetzt **an-zu-fangen**.
- *aufhören* -> Ich versuche, mit dem Rauchen **auf-zu-hören**.
- *vorbereiten* -> Wir haben vor, die Präsentation **vor-zu-bereiten**.

Wann nutzt man **um... zu**, wann **damit**?
- Subjekte sind **identisch**: 👉 Nutze *um... zu* (*Ich lerne Deutsch, **um** Karriere zu machen.*)
- Subjekte sind **unterschiedlich**: 👉 Zwingend *damit* (*Ich lerne Deutsch, **damit** meine Firma expandieren kann.*)

---

## 4. Wortgebrauch: Pronominaladverbien (*da(r)-* & *wo(r)-*)

Viele deutsche Verben verlangen eine feste Präposition (*interessieren für*, *denken an*, *warten auf*).  
Beachte den Unterschied zwischen Personen und Sachen:

| Frage (Sache) | Frage (Person) | Antwort (Sache) | Antwort (Person) |
| :--- | :--- | :--- | :--- |
| **Worauf** wartest du? | Auf **wen** wartest du? | Ich warte **darauf** (dass der Bus kommt). | Ich warte auf **ihn / Anna**. |
| **Woran** denkst du? | An **wen** denkst du? | Ich denke **daran** (an mein Projekt). | Ich denke an **meine Mutter**. |
| **Womit** fährst du? | Mit **wem** fährst du? | Ich fahre **damit** (mit dem Zug). | Ich fahre mit **meinem Freund**. |

*(Wenn die Präposition mit einem Vokal beginnt, wird ein Verbindungslaut `-r-` eingefügt: da**r**an, wo**r**auf).*

---

## 5. Häufige B1-Stolpersteine & Dozenten-Tipps

- ⚠️ **Fehler 1: *als* vs. *wenn* in der Vergangenheit verwechseln.**  
  - **als**: Für ein *einmaliges Ereignis* oder einen *abgeschlossenen Lebensabschnitt* in der Vergangenheit (*Als ich 10 Jahre alt war... / Als ich gestern nach Hause kam...*).  
  - **wenn**: Für *Gegenwart/Zukunft* oder *wiederholte Ereignisse* in der Vergangenheit (*Immer wenn ich Zeit hatte, besuchte ich meine Großeltern*).
- ⚠️ **Fehler 2: Konjunktiv II mit falscher Zeitform bilden.**  
  *Gegenwart:* *Ich **hätte** gern Zeit.* / *Ich **würde** gern reisen.*  
  *Vergangenheit:* *Ich **hätte** das gern **gewusst**.* / *Ich **wäre** gern **gekommen**.* (hätte/wäre + Partizip II).
- ⚠️ **Fehler 3: Das Passiv-Hilfsverb *werden* mit *sein* verwechseln.**  
  *Falsch:* ❌ *Die Tür ist geöffnet von dem Mann.*  
  *Richtig:*  *Die Tür **wird** von dem Mann **geöffnet**.* (Vorgangspassiv = werden + Partizip II).

---

## 6. Dein täglicher B1-Trainingsplan (Der Baukasten)

1. **Passiv-Übung:** Wandle 2 Aktivsätze in das Vorgangspassiv um (Präsens und Präteritum). Achte darauf: Das Akkusativobjekt des Aktivsatzes wird zum Subjekt des Passivsatzes!
2. **Relativsatz-Kette:** Nimm ein Nomen aus [`Cases.md`](file:///e:/Projects/VS_Projects/DeutscheLernen/B1/Cases.md) und bilde dazu 4 Relativsätze (Nominativ, Akkusativ, Dativ und Genitiv).
3. **Hypothesen formulieren:** Bilde 2 Konditionalsätze mit **Konjunktiv II**: *„Wenn ich ..., dann würde/wäre/hätte ich ...“*

---

## 7. Wort- & Konzept-Mapping von B1 auf A2/A1-Grundlagen (Für Einsteiger / Newbies erklärt)

Auf B1 machst du den großen Schritt von der Alltagssprache zur eigenständigen Sprachverwendung. Hier siehst du, wie die neuen B1-Grammatikwörter direkt an das anknüpfen, was du in A1 und A2 gelernt hast:

| Neues B1-Wort / Konzept | Was es auf Englisch bedeutet (Newbie-Erklärung) | Wie du es auf Stufe A1/A2 ausgedrückt hast | Verlinkte Quelle (Vorherige Dateien) |
| :--- | :--- | :--- | :--- |
| **Das Passiv** (*wird gebaut*) | The passive voice ("is being built") | Im Aktiv: *Man baut das Haus* oder *Arbeiter bauen das Haus* | [`A1/Verbs.md`](../A1/Verbs.md) & [`A2/Grammatik.md`](../A2/Grammatik.md) |
| **Der Konjunktiv II** (*hätte, wäre, würde*) | Hypothetical / polite mood ("would have, would be, would do") | Auf A1 nur *ich möchte* (*would like*) oder im Indikativ (*ich will*) | [`A1/Verbs.md`](../A1/Verbs.md) & [`A2/Verbs.md`](../A2/Verbs.md) |
| **Der Genitiv** (*des Vaters, der Mutter*) | Possessive case ("of the father / father's") | Auf A2 mit der Umschreibung: *von dem Vater / von der Mutter* | [`A2/Cases.md`](../A2/Cases.md) & [`A1/Cases.md`](../A1/Cases.md) |
| **Genitivpräpositionen** (*wegen, während, trotz*) | Prepositions demanding Genitive ("because of, during, despite") | Auf A2 mit Nebensätzen: *weil* (wegen), *als/wenn* (während), *aber* (trotz) | [`A2/Grammatik.md`](../A2/Grammatik.md) & [`A2/Prepositions.md`](../A2/Prepositions.md) |
| **Relativpronomen** (*der, die, das, dem...*) | Connecting pronouns ("who, which, that") | Auf A1/A2 wiederholte man das Nomen oder nutzte Personalpronomen (*er/sie/es*) | [`A1/Pronouns.md`](../A1/Pronouns.md) & [`A2/Pronouns.md`](../A2/Pronouns.md) |
| **Infinitiv mit *zu*** (*zu machen*) | "to do something" after verbs like *hoffen, planen* | Auf A1 nutzte man Modalverben (*ich will machen*) | [`A1/Verbs.md`](../A1/Verbs.md) |
| **um... zu / damit** | Purpose clause ("in order to / so that") | Auf A1 nutzte man *wollen* oder *deshalb* | [`A1/Verbs.md`](../A1/Verbs.md) & [`A2/Grammatik.md`](../A2/Grammatik.md) |
| **Pronominaladverbien** (*Worauf / Darauf*) | Prepositional question/pointer words ("what for / for that") | Auf A2 sagte man umgangssprachlich *für was?* oder *auf das* | [`A2/Prepositions.md`](../A2/Prepositions.md) |
| **Doppelkonnektoren** (*sowohl... als auch*) | Two-part connectors ("as well as / not only... but also") | Auf A1/A2 reihte man schlicht mit *und... und...* aneinander | [`A2/Grammatik.md`](../A2/Grammatik.md) |
| **N-Deklination** (*dem Kunden*) | Weak masculine nouns adding `-n` in all cases except Nom. | Auf A1 lernte man die Grundform ohne Endung (*der Kunde*) | [`A1/Nouns.md`](../A1/Nouns.md) |
| **TeKaMoLo** | Formula for ordering adverbs (When? Why? How? Where?) | Auf A1/A2 stellte man Zeit und Ort nach Gefühl auf | [`A1/Grammatik.md`](../A1/Grammatik.md) & [`A2/Grammatik.md`](../A2/Grammatik.md) |

---

🧭 **Navigation & Lernpfad:**  
⬅️ [**Zurück zu A2: A2 Grammatik-Leitfaden (Grammatik.md)**](../A2/Grammatik.md)  
➡️ [**Weiter zu B2: B2 Grammatik-Leitfaden (Grammatik.md)**](../B2/Grammatik.md)

