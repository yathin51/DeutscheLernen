# 📒 A2 Arbeitsbuch & Grammatikskript

> *Deutsch - Aber Hallo! Grammatikübungen A2 — Von Hans Witzlinger (Passau)*

Dieses interaktive Arbeitsbuch enthält sämtliche systematischen Grammatikübungen, Regelerklärungen und Lückentexte der Stufe.
Jede Übung ist didaktisch aufbereitet, mit Beispielen und Hinweisen versehen und enthält aufklappbare Musterlösungen zur Selbstkontrolle.

---

## 📑 Inhaltsübersicht der Lektionen

- [**1.1. Nebensätze - kausal** (S. 3)](#seite-3)
- [**1.2. Verben mit Dativ- und Akkusativobjekt** (S. 4)](#seite-4)
- [**1.3. Präteritum (Imperfekt) - Modalverben** (S. 5)](#seite-5)
- [**2.1. Genitiv** (S. 6)](#seite-6)
- [**2.2. n-Deklination** (S. 7)](#seite-7)
- [**2.3. Deklination des Adjektivs** (S. 8)](#seite-8)
- [**2.3. Deklination des Adjektivs — Vertiefung & Übungen** (S. 9)](#seite-9)
- [**3.1. Nebensätze - dass-Sätze** (S. 10)](#seite-10)
- [**3.2. Vergleichsformen - Steigerung** (S. 11)](#seite-11)
- [**3.2. Vergleichsformen - Steigerung — Vertiefung & Übungen** (S. 12)](#seite-12)
- [**3.3. Reflexive Verben** (S. 13)](#seite-13)
- [**4.1. Verben mit Präpositionalobjekt** (S. 14)](#seite-14)
- [**4.1. Verben mit Präpositionalobjekt — Vertiefung & Übungen** (S. 15)](#seite-15)
- [**4.2. Pronominaladverbien** (S. 16)](#seite-16)
- [**4.2. Pronominaladverbien — Vertiefung & Übungen** (S. 17)](#seite-17)
- [**4.3. Nebensätze - konditional** (S. 18)](#seite-18)
- [**5.1. Konjunktiv II (würde / hätte / wäre / Modalverben)** (S. 19)](#seite-19)
- [**5.2. Präpositionen mit Dativ und Akkusativ** (S. 20)](#seite-20)
- [**5.2. Präpositionen mit Dativ und Akkusativ — Vertiefung & Übungen** (S. 21)](#seite-21)
- [**5.3. Ähnliche Verben** (S. 22)](#seite-22)
- [**6.1. Präteritum (Imperfekt)** (S. 23)](#seite-23)
- [**6.1. Präteritum (Imperfekt) — Vertiefung & Übungen** (S. 24)](#seite-24)
- [**6.2. Relativsätze** (S. 25)](#seite-25)
- [**6.2. Relativsätze — Vertiefung & Übungen** (S. 26)](#seite-26)
- [**6.3. Plusquamperfekt** (S. 27)](#seite-27)
- [**7.1. Nebensätze - damit** (S. 28)](#seite-28)
- [**7.2. Unbestimmte Pronomen und Artikel** (S. 29)](#seite-29)
- [**7.3. Nebensätze - Fragesätze** (S. 30)](#seite-30)
- [**8.1. Demonstrativartikel und Demonstrativpronomen** (S. 31)](#seite-31)
- [**8.2. Futur I** (S. 32)](#seite-32)
- [**8.3. Pauschale Negation** (S. 33)](#seite-33)

---

<a id="seite-3"></a>
## 📄 Seite 3: 1.1. Nebensätze — kausal (weil)

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion: Kausalsätze mit „weil“ (Satzbau & Inversion)**
>
> Kausale Nebensätze begründen eine Aussage und antworten auf die Frage **Warum?** oder **Wieso?**.
>
> 💡 **Hauptsatz vs. Nebensatz:**
> - Im **Hauptsatz** steht das konjugierte Verb strikt auf **Position II**:
>   *Monika **spielt** sehr gut Tennis.*
> - Im **Nebensatz mit „weil“** steht das konjugierte Verb **immer ganz am Ende (Satzende)**:
>   *..., weil sie jeden Tag fleißig **trainiert**.*
>
> | Satztyp | Position I | Position II | Mittelfeld | Satzende |
> | :--- | :--- | :--- | :--- | :--- |
> | **Hauptsatz (HS)** | Monika | **spielt** | gut | Tennis. |
> | **Nebensatz mit „weil“ (NS)** | , weil | sie | viel | **trainiert**. |
>
> ⚠️ **Besondere Verben im Nebensatz:**
> 1. **Trennbare Verben**: Werden am Satzende wieder zusammengeschrieben (*aufgeräumt / aufräumt / zuhört*):
>    - *Paul versteht uns nicht, weil er nie richtig **zuhört**.*
> 2. **Modalverben**: Infinitiv + konjugiertes Modalverb ganz am Ende:
>    - *..., weil sie noch etwas **erledigen muss**.*
> 3. **Perfekt**: Partizip II + konjugiertes Hilfsverb ganz am Ende:
>    - *..., weil er sich **erkältet hat**.*
>
> 🔄 **Vorangestellter Nebensatz (Inversion im Hauptsatz):**
> Beginnt das Satzgefüge mit dem Nebensatz (Position 1), folgt der Hauptsatz sofort mit dem **Verb auf Position 2**:
> - **[Position 1: Weil Max sich unwohl fühlt,]** + **[Position 2: geht]** + **[Position 3: er]** zum Arzt.

### ✍️ Übung 1: Kausalsätze mit „weil“ bilden
*Warum kommen die Leute nicht? Ergänzen Sie das Verb bzw. die Verbgruppe am Satzende.*
> 💡 **Beispiel:** Max hat keine Zeit. ➔ *Max kommt nicht, weil er keine Zeit **hat**.*

- **a)** Beate kommt nicht, weil sie starke Kopfschmerzen _______ *(haben)*.
- **b)** Christian kommt nicht, weil er einen Termin beim Zahnarzt _______ *(haben)*.
- **c)** Inge kommt nicht, weil sie noch etwas _______ *(erledigen müssen)*.
- **d)** Eva kommt nicht, weil sie Besuch von einer Freundin _______ *(bekommen)*.
- **e)** Klaus kommt nicht, weil er für die nächste Prüfung _______ *(lernen müssen)*.
- **f)** Wir kommen nicht, weil wir unseren Onkel _______ *(besuchen)*.
- **g)** Dora kommt nicht, weil sie ihr Fahrrad _______ *(reparieren müssen)*.
- **h)** Carmen kommt nicht, weil sie ihre Wohnung _______ *(aufräumen)*.
- **i)** Ich komme nicht, weil ich ein Referat _______ *(vorbereiten)*.
- **j)** Maria kommt nicht, weil sie ihren Mann vom Flughafen _______ *(abholen)*.
- **k)** Jochen kommt nicht, weil er diese Woche nicht _______ *(ausgehen)*.
- **l)** Du kommst nicht, weil du an einem Kochkurs _______ *(teilnehmen)*.
- **m)** Werner kommt nicht, weil er sich eine neue Wohnung _______ *(ansehen)*.
- **n)** Lisa kommt nicht, weil sie bald nach Wien _______ *(umziehen)*.

### ✍️ Übung 2: Vorangestellte Kausalsätze (Inversion im Hauptsatz)
*Bilden Sie Sätze mit vorangestelltem „Weil“-Nebensatz. Ergänzen Sie das Verb im Hauptsatz auf Position 2.*
> 💡 **Beispiel:** Max fühlt sich unwohl. ➔ *Weil Max sich unwohl fühlt, **geht** er zum Arzt.*

- **a)** Weil Anja einen Flug buchen möchte, _______ *(gehen)* sie zum Reisebüro.
- **b)** Weil Eva mit der Arbeit noch nicht fertig ist, _______ *(können)* sie nicht kommen.
- **c)** Weil Lisa heute Geburtstag hat, _______ *(kaufen)* Paul einen Blumenstrauß.
- **d)** Weil Max Italienisch lernen möchte, _______ *(anmelden)* er sich für einen Sprachkurs an.
- **e)** Weil Robert mit Maria spazieren gehen möchte, _______ *(anrufen)* er sie an.
- **f)** Weil Christian sich erkältet hat, _______ *(können)* er nicht mitkommen.
- **g)** Weil Inge bald eine wichtige Prüfung schreibt, _______ *(lernen)* sie den ganzen Tag.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 3)</strong></summary>

> *Didaktische Lösungen für **1.1. Nebensätze — kausal (weil)**:*

#### Übung 1
- **a)** `hat` — haben (3. Pers. Sg.) am Satzende.
- **b)** `hat` — haben (3. Pers. Sg.) am Satzende.
- **c)** `erledigen muss` — Modalverb am Satzende: Infinitiv + Modalverb.
- **d)** `bekommt` — bekommen (3. Pers. Sg.) am Satzende.
- **e)** `lernen muss` — Modalverb am Satzende: Infinitiv + Modalverb.
- **f)** `besuchen` — besuchen (1. Pers. Pl.) am Satzende.
- **g)** `reparieren muss` — Modalverb am Satzende: Infinitiv + Modalverb.
- **h)** `aufräumt` — Trennbare Verben zusammengeschrieben am Satzende (aufräumen ➔ aufräumt).
- **i)** `vorbereite` — Trennbare Verben zusammengeschrieben (vorbereiten ➔ vorbereite).
- **j)** `abholt` — Trennbare Verben zusammengeschrieben (abholen ➔ abholt).
- **k)** `ausgeht` — Trennbare Verben zusammengeschrieben (ausgehen ➔ ausgeht).
- **l)** `teilnimmst` — Trennbar mit Vokalwechsel (teilnehmen ➔ teilnimmst).
- **m)** `ansieht` — Trennbar mit Vokalwechsel (ansehen ➔ ansieht).
- **n)** `umzieht` — Trennbare Verben zusammengeschrieben (umziehen ➔ umzieht).

#### Übung 2
- **a)** `geht` — Hauptsatz folgt mit Verb auf Position 2 (geht sie).
- **b)** `kann` — Hauptsatz folgt mit Modalverb auf Position 2 (kann sie).
- **c)** `kauft` — Hauptsatz folgt mit Verb auf Position 2 (kauft Paul).
- **d)** `meldet` — Hauptsatz mit trennbarem Verb (meldet er sich ... an).
- **e)** `ruft` — Hauptsatz mit trennbarem Verb (ruft er sie an).
- **f)** `kann` — Hauptsatz folgt mit Modalverb auf Position 2 (kann er).
- **g)** `lernt` — Hauptsatz folgt mit Verb auf Position 2 (lernt sie).

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 3)**](#workbooks:A2:3) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-4"></a>
## 📄 Seite 4: 1.2. Verben mit Dativ- und Akkusativobjekt

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion: Verben mit zwei Objekten (Stellungsregeln)**
>
> Verben des Gebens, Sagens, Zeigens und Sendens verlangen oft ein **Dativobjekt** (Person: *Wem?*) und ein **Akkusativobjekt** (Sache: *Was?*).
>
> 💡 **Die drei Stellungsregeln im Satz:**
>
> 1. **Zwei Nomen als Objekte:**
>    - In der Regel gilt: **Dativ vor Akkusativ** (*Person vor Sache*):
>    - *Der Ober bringt **den Gästen** (Dat.) **die Getränke** (Akk.).*
>    - *Du schenkst **den Kindern** (Dat.) **Schokolade** (Akk.).*
>
> 2. **Nomen + Personalpronomen:**
>    - **Pronomen vor Nomen** (*immer!*):
>    - *Ich gebe **ihr** (Pron.) **das Buch** (Nomen).*
>    - *Ich gebe **es** (Pron.) **der Studentin** (Nomen).*
>
> 3. **Zwei Personalpronomen:**
>    - **Akkusativ vor Dativ** (*Merkformel alphabetisch: A vor D!*):
>    - *Der Ober bringt **es** (Akk.) **ihm** (Dat.).*
>    - *Empfiehlst du Max das Restaurant? — Ja, ich empfehle **es** (Akk.) **ihm** (Dat.).*

### ✍️ Übung 1: Dativobjekt vor Akkusativobjekt
*Setzen Sie die Person im Dativ ein (Dativ vor Akkusativ).*

- **a)** Du schenkst _______ *(Kinder: b)* Schokolade.
- **b)** Ich schreibe _______ *(Freund: b)* eine SMS.
- **c)** Die Lehrerin zeigt _______ *(du: Pronomen)* die Fehler.
- **d)** Der Lehrer erklärt _______ *(wir: Pronomen)* die Grammatik.
- **e)** Claudia schickt _______ *(Tante: b)* eine E-Mail.
- **f)** Die Großmutter erzählt _______ *(Kind: b)* eine Geschichte.
- **g)** Der Minister beantwortet _______ *(Reporterin: u)* die Fragen.
- **h)** Robert bietet _______ *(Freunde: b)* ein Getränk an.
- **i)** Klaus liest _______ *(sie: Pronomen)* den Text vor.
- **j)** Lisa bringt _______ *(wir: Pronomen)* Obst mit.
- **k)** Der Kellner empfiehlt _______ *(ich: Pronomen)* das Steak.
- **l)** Der Dieb stiehlt _______ *(Touristin: u)* die Kamera.

### ✍️ Übung 2: Dativpronomen und Akkusativartikel
*Ergänzen Sie das Personalpronomen im Dativ und den Artikel im Akkusativ.*
> 💡 **Beispiel:** Ich schreibe dir einen Brief.

- **a)** Du gibst _______ *(ich / der)* Schlüssel.
- **b)** Er erzählt _______ *(ich / eine)* Geschichte.
- **c)** Max schickt _______ *(du / ein)* Paket.
- **d)** Ich zeige _______ *(du / das)* Zimmer.
- **e)** Du bringst _______ *(ich / ein)* Orangensaft.
- **f)** Lena erklärt _______ *(du / das)* Problem.

### ✍️ Übung 3: Zwei Personalpronomen (Akkusativ vor Dativ)
*Ersetzen Sie beide Objekte durch Pronomen (Akkusativ vor Dativ).*
> 💡 **Beispiel:** Empfiehlst du Max das Restaurant? ➔ *Ja, ich empfehle **es ihm**.*

- **a)** Liest du mir die Übung vor? ➔ *Ja, ich lese _______ vor.*
- **b)** Schenkst du mir das Bild? ➔ *Ja, ich schenke _______.*
- **c)** Bringst du uns die Pakete mit? ➔ *Ja, ich bringe _______ mit.*
- **d)** Zeigst du Lena das Museum? ➔ *Ja, ich zeige _______.*
- **e)** Erklärst du den Touristen den Weg? ➔ *Ja, ich erkläre _______.*
- **f)** Erzählst du dem Onkel die Geschichte? ➔ *Ja, ich erzähle _______.*
- **g)** Schreibst du mir die Adresse auf? ➔ *Ja, ich schreibe _______ auf.*
- **h)** Schickst du der Tante die Blumen? ➔ *Ja, ich schicke _______.*
- **i)** Bietest du den Freunden deine Hilfe an? ➔ *Ja, ich biete _______ an.*
- **j)** Verkaufst du mir den Wagen? ➔ *Ja, ich verkaufe _______.*

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 4)</strong></summary>

> *Didaktische Lösungen für **1.2. Verben mit Dativ- und Akkusativobjekt**:*

#### Übung 1
- **a)** `den Kindern` — die Kinder (Plural) ➔ Dativ: den Kindern.
- **b)** `dem Freund` — der Freund (maskulin) ➔ Dativ: dem Freund.
- **c)** `dir` — Personalpronomen (du) im Dativ ➔ dir.
- **d)** `uns` — Personalpronomen (wir) im Dativ ➔ uns.
- **e)** `der Tante` — die Tante (feminin) ➔ Dativ: der Tante.
- **f)** `dem Kind` — das Kind (neutral) ➔ Dativ: dem Kind.
- **g)** `einer Reporterin` — eine Reporterin (feminin unbestimmt) ➔ Dativ: einer Reporterin.
- **h)** `den Freunden` — die Freunde (Plural) ➔ Dativ: den Freunden.
- **i)** `ihr` — Personalpronomen (sie) im Dativ ➔ ihr.
- **j)** `uns` — Personalpronomen (wir) im Dativ ➔ uns.
- **k)** `mir` — Personalpronomen (ich) im Dativ ➔ mir.
- **l)** `einer Touristin` — eine Touristin (feminin unbestimmt) ➔ Dativ: einer Touristin.

#### Übung 2
- **a)** `mir den` — Dativ: mir; Akkusativ: den Schlüssel (m).
- **b)** `mir eine` — Dativ: mir; Akkusativ: eine Geschichte (f).
- **c)** `dir ein` — Dativ: dir; Akkusativ: ein Paket (n).
- **d)** `dir das` — Dativ: dir; Akkusativ: das Zimmer (n).
- **e)** `mir einen` — Dativ: mir; Akkusativ: einen Orangensaft (m).
- **f)** `dir das` — Dativ: dir; Akkusativ: das Problem (n).

#### Übung 3
- **a)** `sie dir` — die Übung (f) ➔ sie; mir ➔ dir.
- **b)** `es dir` — das Bild (n) ➔ es; mir ➔ dir.
- **c)** `sie euch` — die Pakete (Pl.) ➔ sie; uns ➔ euch.
- **d)** `es ihr` — das Museum (n) ➔ es; Lena (f) ➔ ihr.
- **e)** `ihn ihnen` — den Weg (m) ➔ ihn; den Touristen (Pl.) ➔ ihnen.
- **f)** `sie ihm` — die Geschichte (f) ➔ sie; dem Onkel (m) ➔ ihm.
- **g)** `sie dir` — die Adresse (f) ➔ sie; mir ➔ dir.
- **h)** `sie ihr` — die Blumen (Pl.) ➔ sie; der Tante (f) ➔ ihr.
- **i)** `sie ihnen` — deine Hilfe (f) ➔ sie; den Freunden (Pl.) ➔ ihnen.
- **j)** `ihn dir` — den Wagen (m) ➔ ihn; mir ➔ dir.

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 4)**](#workbooks:A2:4) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-5"></a>
## 📄 Seite 5: 1.3. Präteritum (Imperfekt) — Modalverben

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> Im gesprochenen Deutsch verwenden wir für die Vergangenheit meistens das **Perfekt** (*„Ich habe gegessen“*). Bei den **Modalverben** (*können, müssen, dürfen, wollen, sollen, mögen*) verwendet man jedoch fast ausnahmslos das **Präteritum** — auch in der Alltagssprache!
>
> **1. Die zwei goldenen Regeln der Modalverben im Präteritum:**
> • **Kein Umlaut**: Alle Umlaute fallen im Präteritum weg! (*können ➔ konnte*, *müssen ➔ musste*, *dürfen ➔ durfte*).
> • **1. und 3. Person Singular sind immer identisch**: *ich konnte = er/sie/es konnte*; *ich musste = er/sie/es musste*. Es gibt keine Endung *-t* in der 3. Person!
> • **Sonderfall „möchte“**: Die Höflichkeitsform *möchte* (Konjunktiv II) hat **kein eigenes Präteritum**. In der Vergangenheit verwendet man immer **wollte** (*„Ich möchte einen Kaffee“ ➔ „Ich wollte einen Kaffee“*).
>
> **2. Konjugationstabelle der Modalverben im Präteritum:**
> | Person | wollen | können | müssen | dürfen | sollen | mögen |
> | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
> | **ich** | wollte | konnte | musste | durfte | sollte | mochte |
> | **du** | wolltest | konntest | musstest | durftest | solltest | mochtest |
> | **er / sie / es** | **wollte** | **konnte** | **musste** | **durfte** | **sollte** | **mochte** |
> | **wir** | wollten | konnten | mussten | durften | sollten | mochten |
> | **ihr** | wolltet | konntet | musstet | durftet | solltet | mochtet |
> | **sie / Sie** | wollten | konnten | mussten | durften | sollten | mochten |
>
> **3. Satzklammer im Präteritum:**
> Das konjugierte Modalverb steht auf **Position 2**, das Vollverb steht im **Infinitiv am Satzende**:
> • *„Gestern **musste** (Pos. 2) ich bis 20 Uhr im Büro **arbeiten** (Satzende).“*
> • *„Ich **wollte** (Pos. 2) meiner Schwester ein Buch **schenken** (Satzende).“*
>
> **🇬🇧 English Cognitive Bridge:**
> • German modal verbs in the past tense correspond cleanly to English equivalents: *konnte* = *could / was able to*, *musste* = *had to*, *durfte* = *was allowed to*, *wollte* = *wanted to*, *sollte* = *was supposed to / should have*.
> • Native speakers almost exclusively use the Präteritum for modal verbs in conversation (*„ich musste“* instead of *„ich habe gemusst“*).

### ✍️ Übung 1: Modalverb im Präteritum einsetzen
*Bilden Sie Sätze im Präteritum. Setzen Sie das angegebene Modalverb in der richtigen Form (3. Person Singular) ein.*

> 💡 **Beispiel:** *(wollen)* ➔ Er **wollte** nach Frankfurt fahren.

- **a)** *(müssen)* Er _______ eine Fahrkarte kaufen.
- **b)** *(wollen)* Er _______ einen Platz reservieren.
- **c)** *(müssen)* Er _______ in Nürnberg umsteigen.
- **d)** *(dürfen)* Er _______ nicht zu spät ankommen.
- **e)** *(können)* Er _______ seinen Freund besuchen.
- **f)** *(sollen)* Er _______ ein Paket mitnehmen.

### ✍️ Übung 2: Sätze ins Präteritum umformen
*Formen Sie die Aussagen ins Präteritum um. Achten Sie auf die richtige Personalform des Modalverbs.*

> 💡 **Beispiel:** Ich kann nicht kommen. ➔ Ich **konnte** nicht kommen.

- **a)** *(Am Wochenende will ich ins Kino gehen.)* Am Wochenende _______ ich ins Kino gehen.
- **b)** *(Leider kann ich nicht mitkommen.)* Leider _______ ich nicht mitkommen.
- **c)** *(Du musst am Samstag arbeiten.)* Du _______ am Samstag arbeiten.
- **d)** *(Wir können das Problem lösen.)* Wir _______ das Problem lösen.
- **e)** *(Ihr sollt den Chef anrufen.)* Ihr _______ den Chef anrufen.
- **f)** *(Ich muss das Paket abholen.)* Ich _______ das Paket abholen.
- **g)** *(Du willst deine Freunde einladen.)* Du _______ deine Freunde einladen.
- **h)** *(Max soll bei der Firma anrufen.)* Max _______ bei der Firma anrufen.
- **i)** *(Du darfst den Zettel nicht zerreißen.)* Du _______ den Zettel nicht zerreißen.
- **j)** *(Ich kann das Fahrrad nicht reparieren.)* Ich _______ das Fahrrad nicht reparieren.
- **k)** *(Wir wollen das Museum besichtigen.)* Wir _______ das Museum besichtigen.
- **l)** *(Du musst den Mülleimer ausleeren.)* Du _______ den Mülleimer ausleeren.
- **m)** *(Ich kann nicht gut schwimmen.)* Ich _______ nicht gut schwimmen.
- **n)** *(Wir müssen die Rechnung bezahlen.)* Wir _______ die Rechnung bezahlen.
- **o)** *(Ihr müsst die U-Bahn nehmen.)* Ihr _______ die U-Bahn nehmen.
- **p)** *(Ich will die Nachrichten sehen.)* Ich _______ die Nachrichten sehen.

### ✍️ Übung 3: Modalverb im Präteritum mit Dativ- und Akkusativobjekt
*Setzen Sie die beiden Objekte (Dativ: Person vor Akkusativ: Sache) in der richtigen Form in die Satzklammer ein.*

> 💡 **Beispiel:** *(wollen · meine Schwester · eine Mail · senden)* ➔ Ich wollte **meiner Schwester eine Mail** senden.

- **a)** *(wollen · mein Bruder · ein Buch · schenken)* Ich wollte _______ schenken.
- **b)** *(sollen · mein Onkel · eine Karte · schreiben)* Ich sollte _______ schreiben.
- **c)** *(können · meine Freunde · die Stadt · zeigen)* Ich konnte _______ zeigen.
- **d)** *(müssen · meine Kollegin · das Problem · erklären)* Ich musste _______ erklären.
- **e)** *(sollen · meine Schwester · der Schlüssel · geben)* Ich sollte _______ geben.
- **f)** *(dürfen · mein Freund · nichts · erzählen)* Ich durfte _______ erzählen.
- **g)** *(wollen · meine Kollegin · das Buch · zurückgeben)* Ich wollte _______ zurückgeben.
- **h)** *(sollen · meine Tante · meine Hilfe · anbieten)* Ich sollte _______ anbieten.
- **i)** *(können · meine Kollegen · der Text · vorlesen)* Ich konnte _______ vorlesen.
- **j)** *(können · mein Freund · das Restaurant · empfehlen)* Ich konnte _______ empfehlen.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 5)</strong></summary>

> *Didaktische Lösungen für **1.3. Präteritum (Imperfekt) — Modalverben**:*

#### Übung 1
- **a)** `musste` — *müssen (3s) ➔ er musste*
- **b)** `wollte` — *wollen (3s) ➔ er wollte*
- **c)** `musste` — *müssen (3s) ➔ er musste*
- **d)** `durfte` — *dürfen (3s) ➔ er durfte*
- **e)** `konnte` — *können (3s) ➔ er konnte*
- **f)** `sollte` — *sollen (3s) ➔ er sollte*

#### Übung 2
- **a)** `wollte` — *Präsens „will“ ➔ Präteritum 1s: wollte*
- **b)** `konnte` — *Präsens „kann“ ➔ Präteritum 1s: konnte*
- **c)** `musstest` — *Präsens „musst“ ➔ Präteritum 2s: musstest*
- **d)** `konnten` — *Präsens „können“ ➔ Präteritum 1p: konnten*
- **e)** `solltet` — *Präsens „sollt“ ➔ Präteritum 2p: solltet*
- **f)** `musste` — *Präsens „muss“ ➔ Präteritum 1s: musste*
- **g)** `wolltest` — *Präsens „willst“ ➔ Präteritum 2s: wolltest*
- **h)** `sollte` — *Präsens „soll“ ➔ Präteritum 3s: sollte*
- **i)** `durftest` — *Präsens „darfst“ ➔ Präteritum 2s: durftest*
- **j)** `konnte` — *Präsens „kann“ ➔ Präteritum 1s: konnte*
- **k)** `wollten` — *Präsens „wollen“ ➔ Präteritum 1p: wollten*
- **l)** `musstest` — *Präsens „musst“ ➔ Präteritum 2s: musstest*
- **m)** `konnte` — *Präsens „kann“ ➔ Präteritum 1s: konnte*
- **n)** `mussten` — *Präsens „müssen“ ➔ Präteritum 1p: mussten*
- **o)** `musstet` — *Präsens „müsst“ ➔ Präteritum 2p: musstet*
- **p)** `wollte` — *Präsens „will“ ➔ Präteritum 1s: wollte*

#### Übung 3
- **a)** `meinem Bruder ein Buch` — *Dativ maskulin (meinem Bruder) + Akkusativ neutral (ein Buch)*
- **b)** `meinem Onkel eine Karte` — *Dativ maskulin (meinem Onkel) + Akkusativ feminin (eine Karte)*
- **c)** `meinen Freunden die Stadt` — *Dativ Plural (meinen Freunden) + Akkusativ feminin (die Stadt)*
- **d)** `meiner Kollegin das Problem` — *Dativ feminin (meiner Kollegin) + Akkusativ neutral (das Problem)*
- **e)** `meiner Schwester den Schlüssel` — *Dativ feminin (meiner Schwester) + Akkusativ maskulin (den Schlüssel)*
- **f)** `meinem Freund nichts` — *Dativ maskulin (meinem Freund) + Indefinitpronomen (nichts)*
- **g)** `meiner Kollegin das Buch` — *Dativ feminin (meiner Kollegin) + Akkusativ neutral (das Buch)*
- **h)** `meiner Tante meine Hilfe` — *Dativ feminin (meiner Tante) + Akkusativ feminin (meine Hilfe)*
- **i)** `meinen Kollegen den Text` — *Dativ Plural (meinen Kollegen) + Akkusativ maskulin (den Text)*
- **j)** `meinem Freund das Restaurant` — *Dativ maskulin (meinem Freund) + Akkusativ neutral (das Restaurant)*

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 5)**](#workbooks:A2:5) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-6"></a>
## 📄 Seite 6: 2.1. Genitiv — Nomen und Artikel

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> Der **Genitiv (2. Fall)** drückt einen **Besitz**, ein **Zugehörigkeitsverhältnis** oder eine nähere Bestimmung aus. Das Fragewort lautet: **Wessen?**
> • *„**Wessen** Koffer ist das?“ ➔ „Das ist der Koffer **des Gastes**.“*
>
> **1. Artikel und Nomen-Endungen im Genitiv:**
> | Genus / Numerus | Bestimmter Artikel | Unbestimmter Artikel | Possessivartikel | Nomen-Endung |
> | :--- | :--- | :--- | :--- | :--- |
> | **Maskulin** | **des** Vaters | **eines** Vaters | **meines** Vaters | **-(e)s** |
> | **Neutral** | **des** Kindes | **eines** Kindes | **meines** Kindes | **-(e)s** |
> | **Feminin** | **der** Mutter | **einer** Mutter | **meiner** Mutter | *(keine)* |
> | **Plural** | **der** Eltern | *(keine)* Eltern | **meiner** Eltern | *(keine)* |
>
> **2. Wann bekommt das Nomen `-es` und wann `-s`?**
> • **Endung `-es`**:
>   1. Bei den meisten **einsilbigen Nomen**: *des Mannes, des Buches, des Flusses, des Liedes, des Tages*.
>   2. Bei Nomen, die auf einen **Zischlaut** enden (*-s, -ss, -ß, -z, -tz, -sch, -x*): *des Platzes, des Arztes, des Schreibtisches*.
> • **Endung `-s`**:
>   1. Bei **mehrsilbigen Nomen**: *des Computers, des Doktors, des Bio-Markts, des Professors*.
>   2. Bei Nomen auf **-el, -en, -er, -ling**: *des Onkels, des Wagens, des Lehrers, des Frühlings*.
>
> **3. Eigennamen von Personen:**
> • Personennamen stehen im Genitiv meist **vor** dem Bezugswort und erhalten ein **-s** (ohne Apostroph):
>   • *„Das ist **Pauls** Fahrrad.“*
>   • *„Das ist **Evas** Waschmaschine.“*
> • Endet der Name bereits auf einen Zischlaut (*-s, -ß, -z, -x*), steht nur ein **Apostroph am Ende**: *„Das ist **Max'** Computer.“*
>
> **🇬🇧 English Cognitive Bridge:**
> • The Genitive case corresponds to the English **apostrophe-s** (*"Eva's car" = „Evas Auto“*) and prepositional **"of"** phrases (*"the capital of the country" = „die Hauptstadt des Landes“*).
> • **Crucial difference**: In German, feminine (*der Frau*) and plural (*der Kinder*) nouns **never take an -s on the noun itself**; only the article changes to *der*!

### ✍️ Übung 1: Genitiv im Satz bilden
*Bilden Sie Sätze mit Genitiv. Achten Sie auf die richtige Artikel- und Nomenendung (-(e)s bei Maskulinum und Neutrum) bzw. den Genitiv bei Eigennamen.*

> 💡 **Beispiel:** *(der Mann)* ➔ Die Jacke **des Mannes** ist schwarz.

- **a)** *(der Fluss)* Das Wasser _______ ist schmutzig.
- **b)** *(die Studentin)* Das Apartment _______ ist klein.
- **c)** *(die Geschichte)* Das Ende _______ ist traurig.
- **d)** *(das Schulkind)* Die Eltern _______ sind krank.
- **e)** *(der Professor)* Der Garten _______ ist groß.
- **f)** *(die Gäste)* Das Gepäck _______ ist schwer.
- **g)** *(Eva)* _______ Waschmaschine ist kaputt.
- **h)** *(der Bio-Markt)* Die Preise _______ sind hoch.
- **i)** *(das Problem)* Die Lösung _______ ist schwierig.

### ✍️ Übung 2: Genitiv mit bestimmtem Artikel
*Ergänzen Sie den Genitiv des angegebenen Nomens mit dem bestimmten Artikel (des / der).*

> 💡 **Beispiel:** *(das Buch)* ➔ Kennst du den Titel **des Buches**?

- **a)** *(die Bank)* Kennst du die Direktorin _______?
- **b)** *(der Roman)* Kennst du den Autor _______?
- **c)** *(das Lied)* Kennst du den Text _______?
- **d)** *(der Minister)* Kennst du die Frau _______?
- **e)** *(die Stadt)* Kennst du das Zentrum _______?
- **f)** *(die Studentin)* Kennst du den Vater _______?
- **g)** *(das Land)* Kennst du die Hauptstadt _______?
- **h)** *(die Vorlesung)* Kennst du das Thema _______?
- **i)** *(die Nachbarin)* Kennst du den Sohn _______?

### ✍️ Übung 3: Possessivartikel im Genitiv (Wessen ...?)
*Beantworten Sie die Frage nach dem Besitzer (Wessen ...?) mit dem passenden Possessivartikel im Genitiv.*

> 💡 **Beispiel:** *(mein Bruder)* ➔ Wessen Hut ist das? — Das ist der Hut **meines Bruders**.

- **a)** *(sein Vater)* Wessen Wagen ist das? — Das ist der Wagen _______.
- **b)** *(unsere Tante)* Wessen Brille ist das? — Das ist die Brille _______.
- **c)** *(ihr Großvater)* Wessen Hund ist das? — Das ist der Hund _______.
- **d)** *(eure Gäste)* Wessen Gepäck ist das? — Das ist das Gepäck _______.
- **e)** *(ihre Tochter)* Wessen Stifte sind das? — Das sind die Stifte _______.
- **f)** *(mein Freund)* Wessen Computer ist das? — Das ist der Computer _______.
- **g)** *(dein Onkel)* Wessen Schirm ist das? — Das ist der Schirm _______.
- **h)** *(meine Kollegin)* Wessen Auto ist das? — Das ist das Auto _______.
- **i)** *(sein Sohn)* Wessen Bücher sind das? — Das sind die Bücher _______.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 6)</strong></summary>

> *Didaktische Lösungen für **2.1. Genitiv — Nomen und Artikel**:*

#### Übung 1
- **a)** `des Flusses` — *Maskulin Genitiv: des + Fluss + -es ➔ des Flusses*
- **b)** `der Studentin` — *Feminin Genitiv: der Studentin*
- **c)** `der Geschichte` — *Feminin Genitiv: der Geschichte*
- **d)** `des Schulkindes` — *Neutral Genitiv: des + Schulkind + -es ➔ des Schulkindes*
- **e)** `des Professors` — *Maskulin Genitiv: des + Professor + -s ➔ des Professors*
- **f)** `der Gäste` — *Plural Genitiv: der Gäste*
- **g)** `Evas` — *Eigenname vor dem Nomen mit -s: Evas Waschmaschine*
- **h)** `des Bio-Marktes` — *Maskulin Genitiv: des + Bio-Markt + -es ➔ des Bio-Marktes*
- **i)** `des Problems` — *Neutral Genitiv: des + Problem + -s ➔ des Problems*

#### Übung 2
- **a)** `der Bank` — *die Bank (f) ➔ Genitiv: der Bank*
- **b)** `des Romans` — *der Roman (m) ➔ Genitiv: des Romans*
- **c)** `des Liedes` — *das Lied (n) ➔ Genitiv: des Liedes*
- **d)** `des Ministers` — *der Minister (m) ➔ Genitiv: des Ministers*
- **e)** `der Stadt` — *die Stadt (f) ➔ Genitiv: der Stadt*
- **f)** `der Studentin` — *die Studentin (f) ➔ Genitiv: der Studentin*
- **g)** `des Landes` — *das Land (n) ➔ Genitiv: des Landes*
- **h)** `der Vorlesung` — *die Vorlesung (f) ➔ Genitiv: der Vorlesung*
- **i)** `der Nachbarin` — *die Nachbarin (f) ➔ Genitiv: der Nachbarin*

#### Übung 3
- **a)** `seines Vaters` — *sein Vater (m) ➔ Genitiv: seines Vaters*
- **b)** `unserer Tante` — *unsere Tante (f) ➔ Genitiv: unserer Tante*
- **c)** `ihres Großvaters` — *ihr Großvater (m) ➔ Genitiv: ihres Großvaters*
- **d)** `eurer Gäste` — *eure Gäste (Pl.) ➔ Genitiv: eurer Gäste*
- **e)** `ihrer Tochter` — *ihre Tochter (f) ➔ Genitiv: ihrer Tochter*
- **f)** `meines Freundes` — *mein Freund (m) ➔ Genitiv: meines Freundes*
- **g)** `deines Onkels` — *dein Onkel (m) ➔ Genitiv: deines Onkels*
- **h)** `meiner Kollegin` — *meine Kollegin (f) ➔ Genitiv: meiner Kollegin*
- **i)** `seines Sohnes` — *sein Sohn (m) ➔ Genitiv: seines Sohnes*

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 6)**](#workbooks:A2:6) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-7"></a>
## 📄 Seite 7: 2.2. n-Deklination (Schwache Nomen)

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> Die **n-Deklination** betrifft eine spezielle Gruppe **maskuliner Nomen** (sowie eine einzige Ausnahme im Neutrum: *das Herz*), die in allen Fällen außer dem Nominativ Singular die Endung **`-n` oder `-en`** erhalten.
>
> **1. Übersicht: Starke vs. Schwache Deklination:**
> | Kasus | Starkes Maskulinum (*der Arzt*) | Schwaches Maskulinum (*der Patient*) | Plural (*die Patienten*) |
> | :--- | :--- | :--- | :--- |
> | **Nominativ** | der Arzt | **der Patient** | die Patienten |
> | **Akkusativ** | den Arzt | **den Patienten** | die Patienten |
> | **Dativ** | dem Arzt | **dem Patienten** | den Patienten |
> | **Genitiv** | des Arztes *(+es)* | **des Patienten** *(kein -s!)* | der Patienten |
>
> **2. Typische Gruppen der n-Deklination:**
> • **Maskuline Nomen auf `-e`** (Personen, Tiere, Nationalitäten):
>   • *der Junge, der Kollege, der Kunde, der Neffe, der Zeuge, der Experte*
>   • *der Löwe, der Hase, der Affe, der Falke*
>   • *der Pole, der Russe, der Franzose, der Kroate, der Däne*
> • **Maskuline Fremdwörter mit bestimmten Endungen** (meist Personen oder Berufe):
>   • **`-ist`**: *der Tourist, der Polizist, der Journalist, der Gitarrist, der Spezialist*
>   • **`-ent / -ant`**: *der Student, der Patient, der Präsident, der Praktikant, der Diamant, der Elefant*
>   • **`-at / -et / -it / -ot`**: *der Soldat, der Automat, der Komet, der Planet, der Satellit, der Pilot*
>   • **`-oge`**: *der Biologe, der Pädagoge, der Psychologe, der Geologe*
> • **Weitere wichtige schwache Maskulina**:
>   • *der Mensch, der Nachbar, der Herr, der Bär, der Prinz, der Fotograf, der Architekt, der Jurist*
>
> **3. Wichtige Ausnahmen:**
> • **`der Herr`**: Erhält im Singular nur **`-n`**, im Plural jedoch **`-en`**:
>   • *den Herrn, dem Herrn, des Herrn* (Sg.) ➔ *die / den / der Herren* (Pl.).
> • **`das Herz`** (einziges Neutrum):
>   • *das Herz (Nom.), das Herz (Akk.), dem Herzen (Dat.), des Herzens (Gen. mit -ens!)*.
>
> **🇬🇧 English Cognitive Bridge:**
> • English has no weak nouns today, but historical plurals like *ox ➔ oxen* and *child ➔ children* share the same Germanic root suffix *-en*.
> • **The Golden Rule**: If a German noun belongs to the n-declension and is **not the grammatical subject (Nominativ)**, you must attach **`-n` or `-en`**!

### ✍️ Übung 1: n-Deklination im Akkusativ einsetzen
*Setzen Sie das maskuline Nomen mit der passenden Endung (-n bzw. -en) im Akkusativ ein.*

> 💡 **Beispiel:** *(der Junge)* ➔ Wie heißt der Junge? — Ich kenne den **Jungen** nicht.

- **a)** *(der Kollege)* Wie heißt der Kollege? — Ich kenne den _______ nicht.
- **b)** *(der Student)* Wie heißt der Student? — Ich kenne den _______ nicht.
- **c)** *(der Tourist)* Wie heißt der Tourist? — Ich kenne den _______ nicht.
- **d)** *(der Postbote)* Wie heißt der Postbote? — Ich kenne den _______ nicht.
- **e)** *(der Patient)* Wie heißt der Patient? — Ich kenne den _______ nicht.
- **f)** *(der Polizist)* Wie heißt der Polizist? — Ich kenne den _______ nicht.
- **g)** *(der Soldat)* Wie heißt der Soldat? — Ich kenne den _______ nicht.
- **h)** *(der Praktikant)* Wie heißt der Praktikant? — Ich kenne den _______ nicht.
- **i)** *(der Kunde)* Wie heißt der Kunde? — Ich kenne den _______ nicht.
- **j)** *(der Agent)* Wie heißt der Agent? — Ich kenne den _______ nicht.
- **k)** *(der Fotograf)* Wie heißt der Fotograf? — Ich kenne den _______ nicht.
- **l)** *(der Architekt)* Wie heißt der Architekt? — Ich kenne den _______ nicht.
- **m)** *(der Herr)* Wie heißt der Herr? — Ich kenne den _______ nicht.
- **n)** *(der Jurist)* Wie heißt der Jurist? — Ich kenne den _______ nicht.
- **o)** *(der Nachbar)* Wie heißt der Nachbar? — Ich kenne den _______ nicht.

### ✍️ Übung 2: n-Deklination im Kontext (Akkusativ vs. Dativ)
*Bilden Sie Fragen mit dem passenden Artikel und Nomen der n-Deklination. Achten Sie darauf, ob das Verb bzw. die Präposition den Akkusativ (den ...) oder Dativ (dem ...) verlangt.*

> 💡 **Beispiel:** *(der Experte · fragen)* ➔ Fragst du **den Experten**?

- **a)** *(der Student · helfen)* Hilfst du _______?
- **b)** *(der Gitarrist · kennen)* Kennst du _______?
- **c)** *(der Nachbar · besuchen)* Besuchst du _______?
- **d)** *(der Diamant · kaufen)* Kaufst du _______?
- **e)** *(der Automat · reparieren)* Reparierst du _______?
- **f)** *(der Journalist · antworten)* Antwortest du _______?
- **g)** *(der Postbote · sprechen mit)* Sprichst du mit _______?
- **h)** *(der Elefant · fotografieren)* Fotografierst du _______?
- **i)** *(der Polizist · anrufen)* Rufst du _______ an?
- **j)** *(der Kollege · einladen)* Lädst du _______ ein?
- **k)** *(der Experte · glauben)* Glaubst du _______?
- **l)** *(der Präsident · erkennen)* Erkennst du _______?
- **m)** *(der Junge · aufwecken)* Weckst du _______ auf?
- **n)** *(der Komet · sehen)* Siehst du _______?
- **o)** *(Herr Berg · vertrauen)* Vertraust du _______?

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 7)</strong></summary>

> *Didaktische Lösungen für **2.2. n-Deklination (Schwache Nomen)**:*

#### Übung 1
- **a)** `Kollegen` — *der Kollege (-e) ➔ Akkusativ: den Kollegen*
- **b)** `Studenten` — *der Student (-ent) ➔ Akkusativ: den Studenten*
- **c)** `Touristen` — *der Tourist (-ist) ➔ Akkusativ: den Touristen*
- **d)** `Postboten` — *der Postbote (-e) ➔ Akkusativ: den Postboten*
- **e)** `Patienten` — *der Patient (-ent) ➔ Akkusativ: den Patienten*
- **f)** `Polizisten` — *der Polizist (-ist) ➔ Akkusativ: den Polizisten*
- **g)** `Soldaten` — *der Soldat (-at) ➔ Akkusativ: den Soldaten*
- **h)** `Praktikanten` — *der Praktikant (-ant) ➔ Akkusativ: den Praktikanten*
- **i)** `Kunden` — *der Kunde (-e) ➔ Akkusativ: den Kunden*
- **j)** `Agenten` — *der Agent (-ent) ➔ Akkusativ: den Agenten*
- **k)** `Fotografen` — *der Fotograf (-af) ➔ Akkusativ: den Fotografen*
- **l)** `Architekten` — *der Architekt (-ekt) ➔ Akkusativ: den Architekten*
- **m)** `Herrn` — *der Herr (Sonderform) ➔ Akkusativ: den Herrn*
- **n)** `Juristen` — *der Jurist (-ist) ➔ Akkusativ: den Juristen*
- **o)** `Nachbarn` — *der Nachbar (-ar) ➔ Akkusativ: den Nachbarn*

#### Übung 2
- **a)** `dem Studenten` — *helfen verlangt Dativ ➔ dem Studenten*
- **b)** `den Gitarristen` — *kennen verlangt Akkusativ ➔ den Gitarristen*
- **c)** `den Nachbarn` — *besuchen verlangt Akkusativ ➔ den Nachbarn*
- **d)** `den Diamanten` — *kaufen verlangt Akkusativ ➔ den Diamanten*
- **e)** `den Automaten` — *reparieren verlangt Akkusativ ➔ den Automaten*
- **f)** `dem Journalisten` — *antworten verlangt Dativ ➔ dem Journalisten*
- **g)** `dem Postboten` — *mit verlangt Dativ ➔ dem Postboten*
- **h)** `den Elefanten` — *fotografieren verlangt Akkusativ ➔ den Elefanten*
- **i)** `den Polizisten` — *anrufen verlangt Akkusativ ➔ den Polizisten*
- **j)** `den Kollegen` — *einladen verlangt Akkusativ ➔ den Kollegen*
- **k)** `dem Experten` — *glauben verlangt Dativ ➔ dem Experten*
- **l)** `den Präsidenten` — *erkennen verlangt Akkusativ ➔ den Präsidenten*
- **m)** `den Jungen` — *aufwecken verlangt Akkusativ ➔ den Jungen*
- **n)** `den Kometen` — *sehen verlangt Akkusativ ➔ den Kometen*
- **o)** `Herrn Berg` — *vertrauen verlangt Dativ; Anrede Herr ➔ Herrn Berg*

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 7)**](#workbooks:A2:7) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-8"></a>
## 📄 Seite 8: 2.3. Deklination des Adjektivs

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 2.3. Deklination des Adjektivs
> • Die Erdbeeren schmecken süß. ➔ adverbial
> • Die Erdbeeren sind süß. ➔ prädikativ
> • Adjektive haben adverbial (bei Verben) und prädikativ (z. B. bei sein) keine Endung.


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Das ist ein neuer Computer. Das ist der neue Computer. ➔  attributiv
> • Adjektive haben attributiv (vor Nomen) in der Regel eine Endung.
> • Frischer Orangensaft ist gesund. / Jana macht sich große Sorgen. / Zeig mir mal dein neues Fahrrad.
> • Kennst du die neuen Regeln? / Gib mir bitte das blaue T-Shirt. / Ich arbeite mit einem neuen Programm.
> • Tabelle 1 (starke Endungen) Tabelle 2 (schwache Endungen)
> • mask. fem. neut. Pl.
> • 1. Beispiel: (Tab. 1 + Tab. 2)
> • mit eine  m neuen Hut
> • 2. Beispiel: (O + Tab. 1)
> • für eino gesundes Leben
> • mask. fem. neut. Pl.
> • Nom. -er O -e -es O -e Nom. -e -e -e -en
> • Akk. -en -e -es O -e Akk. -en -e -e -en
> • Dat. -em -er -em -en Dat. -en -en -en -en
> • Gen. -es*-en -er -es *-en -er Gen. -en -en -en -en
> • Tabelle 1 enthält die Endungen, die von den bestimmten Artikeln etc. bekannt sind.
> • Tabelle 2 braucht man für die Adjektive, wenn Tabelle 1 vorher schon benutzt ist.
> • Artikel: ein, kein, mein, dein, sein, unser, euer, ihr ➔➔ keine Endung: Nom. mask. und neutr. + Akk. neutr. ➔➔ o
> • kein Arti kel ➔ Adj ekt i v: mit  frisch__  Obst
> • 
> • Dat.
> • 
> • Tab. 1
> • 
> • neutr.
> • Ar ti kel  mit Endung ➔ Ad j ekti v: für mein__ alt__ Freunde
> • 
> • Akk.
> • 
> • Tab. 1
> • 
> • Tab. 2
> • 
> • Pl.
> • Ar ti kel  ohne Endung ➔ Ad jekti v: ein groß__ Fehler
> • 
> • Nom.  o    
> • Tab. 1
> • 
> • mask.
> • * Achtung:  Adjektive im Genitiv
> • Sg. mask. + neutr. immer (auch ohne Artikel) Tab. 2
> • wegen stark__* Sturmes
> • 
> • Gen.
> • 
> • Tab. 2
> • 
> • mask.


### ✍️ Übung 1
*Wählen Sie aus dem Wortkasten: schnell, langweilig, sauer, spannend, gut, langsam, unfreundlich, preiswert, nett*

> 💡 **Beispiel:** Wie schmeckt der Apfel?   Er schmeckt sauer.

- **a)** Wie ist der Film?
- **b)** Wie ist das Wetter in Hamburg?
- **c)** Wie findest du das Hotelzimmer?
- **d)** Wie spricht Peter?
- **e)** Wie gefällt dir Marias Kleid?
- **f)** Wie findest du den Film?
- **g)** Wie fährt Julia Auto?
- **h)** Wie ist sein Bruder?
- **i)** Wie findest du den Roman?

### ✍️ Übung 2
*Wählen Sie aus dem Wortkasten: Akk., Tab., Tab.*

> 💡 **Beispiel:** Fahrrad - blau   Kaufst du da      s             blau      e       Fahrrad?

- **a)** Kleid — rot
- **b)** Jacke — schwarz
- **c)** Koffer — groß
- **d)** Tasche — klein
- **e)** Pullover — grün
- **f)** Mütze — rot
- **g)** Hose — blau
- **h)** Wein — süß
- **i)** Schuhe — braun
- **j)** Mantel — dunkelblau
- **k)** Hemd — weiß
- **l)** *(Orangen)*  — spanisch

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 8)</strong></summary>

> *Didaktische Lösungen für **2.3. Deklination des Adjektivs**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `Orangen` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 8)**](#workbooks:A2:8) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-9"></a>
## 📄 Seite 9: 2.3. Deklination des Adjektivs — Vertiefung & Übungen

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • -er sauber ➔ saub[e]res Wasser
> • -en offen ➔ off[e]ne Fenster
> • -el edel ➔ edler Wein (nicht: edeler)


### ✍️ Übung 3
> 💡 **Beispiel:** Auto   Schau, das ist    mein neu      es             Auto!

- **Nom.**  Tab. 1
- **a)** Fahrrad
- **b)** Tasche
- **c)** Uhr
- **d)** Computer
- **e)** Schuhe
- **f)** Brille
- **g)** Zimmer
- **h)** Bücher
- **i)** Roller
- **j)** Hund
- **k)** Wagen
- **l)** Schreibtisch
- **m)** Schrank
- **n)** Kamera
- **o)** Kaffeemaschine
- **p)** Lampe
- **q)** Sofa
- **r)** Ohrringe

### ✍️ Übung 4
> 💡 **Beispiel:** Roman - spannend   Ist   der   Roman spannend? - Nein, das ist   ein   langweilig  er   Roman.

- **Nom.**  Tab. 1
- **a)** Arbeit — schwierig
- **b)** Film — lustig
- **c)** Beispiel — schlecht
- **d)** Jacke — dick
- **e)** Hotelzimmer — groß
- **f)** Lexikon — teuer
- **g)** Verkäuferin — freundlich
- **h)** *(Wagen)*  — alt
- **i)** Bluse — schön
- **j)** Paket — schwer
- **k)** Mann — reich
- **l)** Brot — alt
- **m)** Sofa — bequem
- **n)** Apfel — sauer
- **o)** Hemd — schmutzig

### ✍️ Übung 5
*Wählen Sie aus dem Wortkasten: Nom., Tab., Nom., Tab.1, Tab.*

> 💡 **Beispiel:** Brot - alt   Schmeckt dir alt  es   Brot? - Nein,  da  s    alt  e      Brot schmeckt mir nicht  .

- **a)** Wein — trocken !
- **b)** *(Kuchen)*  — süß
- **c)** Kaffee — stark
- **d)** Tee — schwarz
- **e)** Bier — dunkel !
- **f)** Fisch — fettig
- **g)** Limonade — sauer !
- **h)** Wurst — scharf
- **i)** Schokolade — bitter !
- **j)** Suppe — kalt
- **k)** Mineralwasser — still
- **l)** Butter — salzig

### ✍️ Übung 6
*Wählen Sie aus dem Wortkasten: hoch, hoh-, hohe, Kosten*

> 💡 **Beispiel:** Pullover - grün   Gefällt dir de  r   grün  e     Pullover? - Ja, ich möchte mir d  e  n   grün  en   Pullover kaufen.

- **a)** Tisch — modern
- **b)** Uhr — golden !
- **c)** Mantel — elegant
- **d)** *(Wagen)*  — sportlich
- **e)** Koffer — groß
- **f)** Fahrrad — teuer !
- **g)** Anzug — grau
- **h)** Sessel — alt
- **i)** Jacke — wollen !
- **j)** Spiegel — antik
- **k)** Schuhe — modisch
- **l)** Vase — hoch !

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 9)</strong></summary>

> *Didaktische Lösungen für **2.3. Deklination des Adjektivs — Vertiefung & Übungen**:*

#### Übung 3
- **Nom.** `—` — Grammatische Ergänzung
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung
- **o)** `—` — Grammatische Ergänzung
- **p)** `—` — Grammatische Ergänzung
- **q)** `—` — Grammatische Ergänzung
- **r)** `—` — Grammatische Ergänzung

#### Übung 4
- **Nom.** `—` — Grammatische Ergänzung
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `Wagen` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung
- **o)** `—` — Grammatische Ergänzung

#### Übung 5
- **a)** `—` — Grammatische Ergänzung
- **b)** `Kuchen` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

#### Übung 6
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `Wagen` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 9)**](#workbooks:A2:9) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-10"></a>
## 📄 Seite 10: 3.1. Nebensätze - dass-Sätze

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Lektion 3
> • 3.1. Nebensätze - dass-Sätze
> • sagen, erklären, behaupten, erzählen, denken, glauben, meinen, planen,
> • erwarten,  annehmen, hören, fühlen, wünschen, hoffen, befürchten


### ✍️ Übung 1
*Wählen Sie aus dem Wortkasten: Ja, Paul, hat, es, versprochen, dass, er, mich, mitnimmt.*

> 💡 **Beispiel:** Nimmt Paul dich mit? Hat er es versprochen?

- **a)** Bekommt Max diese Stelle? Hat er es gesagt?
- **b)** Hilft Lisa euch bei der Renovierung? Hat sie es angeboten?
- **c)** Kann Karl an dem Kurs teilnehmen? Weiß er es?
- **d)** Kann Eva die Prüfung wiederholen? Hofft sie es?
- **e)** Besucht Carlos seinen Freund in Madrid? Plant er es?
- **f)** Holt Martin Maria vom Bahnhof ab? Erwartet sie es?
- **g)** Besucht Robert seine Tante im Krankenhaus? Hat er es dir erzählt?
- **h)** Muss Lena den Wagen in die Werkstatt bringen? Glaubt sie es?

### ✍️ Übung 2
> 💡 **Beispiel:** Was ist sicher? - Pavel muss bald abreisen. > Es ist sicher,       dass       Pavel bald abreisen muss.

- **a)** Was ist schade? — Lena hat keine Zeit.
- **b)** Was ist notwendig? — Du bringst dein Auto in die Werkstatt.
- **c)** Was ärgert dich? — Max kommt immer zu spät.
- **d)** Was ist möglich? — Carmen ist nach Köln gefahren.
- **e)** Was ist wichtig? — Paul schafft die Prüfung.
- **f)** Was gefällt dir nicht? — Ich muss so viel arbeiten.
- **g)** Was tut dir leid? — Ich kann dich nicht mitnehmen.
- **h)** Was freut dich? — Meine Kolleg*innen haben mir zum Geburtstag gratuliert.
- **i)** Was kann sein? — Heute Abend kommt ein Sturm.

### ✍️ Übung 3
> 💡 **Beispiel:** sich erkälten - Karl   Stimmt es, dass Karl sich erkältet hat?

- **a)** Auto — kaufen — du
- **b)** Haus — mieten — Hanna
- **c)** Unfall — haben — Paul
- **d)** Prüfung — bestehen — alle
- **e)** Termin — vergessen — Thomas
- **f)** Jacke — verlieren — du
- **g)** Vertrag — unterschreiben — Clara
- **h)** *(heiraten)*  — Max und Eva
- **i)** Rom — fliegen — ihr
- **j)** *(Tunesien)*  — sein — Julia

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 10)</strong></summary>

> *Didaktische Lösungen für **3.1. Nebensätze - dass-Sätze**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `heiraten` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `Tunesien` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 10)**](#workbooks:A2:10) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-11"></a>
## 📄 Seite 11: 3.2. Vergleichsformen - Steigerung

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 3.2. Vergleichsformen - Steigerung
> • Es gibt zwei Steigerungsformen: Komparativ und Superlativ
> • Steigerungsformen normalerweise nur bei Adjektiven.
> • schön ➔ schöner ➔ am schönsten
> • klein ➔ kleiner ➔ am kleinsten
> • Steigerungsformen bei einigen Adverbien
> • wohl ➔ wohler ➔ am wohlsten
> • oft ➔ öfter ➔ am öftesten
> • bald ➔ eher ➔ am ehesten
> • gern ➔ lieber ➔ am liebsten
> • Steigerungsformen bei den indefiniten Zahlwörtern viel und wenig
> • viel ➔ mehr ➔ am meisten
> • wenig ➔ weniger ➔ am wenigsten
> • Steigerung - Komparativ
> • prädikativ prädikativ
> • Das Flugzeug ist recht schnell, aber die Rakete ist viel schneller.
> • Komparativ  ➔  -er


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Immer  Umlaut bei:
> • alt, arm, hart, kalt, krank, lang, nah, scharf, schwach, stark, warm, groß, hoch, dumm, jung, klug, kurz
> • Umlaut möglich bei: z. B. blass, glatt, nass, gesund
> • bitter ➔ bitt[e]rer
> • Bei Adjektiven auf -er, -en  kann im Komparativ das e wegfallen.
> • Bei Adjektiven auf -el  fällt im Komparativ das e weg.
> • trocken ➔ trock[e]ner
> • dunkel ➔ dunkler
> • Adjektive mit unregelmäßigen Komparativformen:
> • hoch ➔ höher gut ➔ besser


### ✍️ Übung 1
> 💡 **Beispiel:** Zimmer - klein   Mein Zimmer ist klein  er       als dein Zimmer.

- **a)** Wörterbuch — dick
- **b)** *(Wagen)*  — schnell
- **c)** Schal — schön
- **d)** Reise — gefährlich
- **e)** Koffer — schwer
- **f)** Zimmer — hell
- **g)** Wohnung — billig
- **h)** Arbeit — schwierig
- **i)** Chef — nett
- **j)** Bruder — schlau
- **k)** Fahrrad — modern
- **l)** Onkel — reich

### ✍️ Übung 2
> 💡 **Beispiel:** Getränk - kalt   Dieses Getränk ist k  ä  lt  er       als das andere.

- **a)** Sofa — alt
- **b)** Raum — groß
- **c)** Stadt — nah
- **d)** Apfel — sauer
- **e)** Mantel — lang
- **f)** Suppe — scharf
- **g)** *(Essen)*  — gesund
- **h)** Geschichte — kurz
- **i)** Espresso — stark
- **j)** Stein — hart
- **k)** Baum — hoch
- **l)** Film — gut
- **m)** Wein — trocken
- **n)** Pullover — warm
- **o)** Hund — klug
- **p)** Farbe — dunkel

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 11)</strong></summary>

> *Didaktische Lösungen für **3.2. Vergleichsformen - Steigerung**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `Wagen` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `Essen` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung
- **o)** `—` — Grammatische Ergänzung
- **p)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 11)**](#workbooks:A2:11) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-12"></a>
## 📄 Seite 12: 3.2. Vergleichsformen - Steigerung — Vertiefung & Übungen

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Steigerung - Superlativ
> • prädikativ / adverbial: am  -sten einfach ➔ am einfachsten
> • prädikativ
> • Positiv Die Bohnen sind billig.
> • Komparativ Die Tomaten sind billiger.
> • Superlativ Die Gurken sind am billig sten .
> • Umlaut beim Superlativ ➔ Adjektive wie beim Komparativ, z. B.:
> • warm - am wärmsten klug - am klügsten


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • nah - am nächsten gut - am besten
> • Achtung: nach s, ss, ß, z, t  ➔  -esten
> • z. B. süß - am süßesten hart - am härtesten
> • kurz - am kürzesten harmlos - am harmlosesten
> • blass - am blassesten intelligent - am intelligentesten
> • aber:     groß   - am   größ  ten


### ✍️ Übung 3
> 💡 **Beispiel:** Die Wohnung ist so billig wie das Büro. -  Nein, das Büro ist billig  er   als die Wohnung.

- **a)** Unser Haus ist so groß wie euer Haus.
- **b)** Frage 1 ist so schwierig wie Frage 2.
- **c)** Das Bier ist so kalt wie der Weißwein.
- **d)** Die Äpfel sind so teuer wie die Tomaten.
- **e)** Paul ist so fleißig wie Carmen.
- **f)** Das Rathaus ist so alt wie die Kirche.
- **g)** Das Steak ist so gut wie das Schnitzel.
- **h)** Das Gebäude ist so hoch wie dieser Turm.
- **i)** Mein Onkel ist so reich wie deine Tante.
- **j)** Klaus verdient so viel wie Maria

### ✍️ Übung 4
*Wählen Sie aus dem Wortkasten: Adjektive, mit, unregelmäßigen, Superlativformen:*

> 💡 **Beispiel:** Hotel - billig    Dieses Hotel ist   am   billig  sten  .

- **a)** Flugzeug — schnell
- **b)** Koffer — schwer
- **c)** Sofa — bequem
- **d)** Film — langweilig
- **e)** Kleid — schön
- **f)** Restaurant — teuer
- **g)** Jacke — warm
- **h)** Text — lang
- **i)** Suppe — scharf
- **j)** Leute — arm
- **k)** Argument — dumm
- **l)** Kinder — klug

### ✍️ Übung 5
> 💡 **Beispiel:** Stadtteil - Wohnungen - teuer   In diesem Stadtteil sind die Wohnungen   am     teuer      sten      .

- **a)** Hotel — Zimmer — groß
- **b)** Land — Berge — hoch
- **c)** Geschäft — Personal — freundlich
- **d)** Museum — Bilder — interessant
- **e)** Monat — Nächte — kurz
- **f)** *(Laden)*  — Gemüse — frisch
- **g)** Restaurant — Essen — lecker
- **h)** *(Kindergarten)*  — Kinder — jung
- **i)** Jahreszeit — Tage — heiß
- **j)** Bäckerei — Brot — gut

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 12)</strong></summary>

> *Didaktische Lösungen für **3.2. Vergleichsformen - Steigerung — Vertiefung & Übungen**:*

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung

#### Übung 4
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

#### Übung 5
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `Laden` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `Kindergarten` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 12)**](#workbooks:A2:12) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-13"></a>
## 📄 Seite 13: 3.3. Reflexive Verben

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 3.3. Reflexive Verben
> • Manche Verben sind immer reflexiv.
> • Man kann das Pronomen nicht weglassen und nicht ersetzen.
> • immer reflexiv: Ich beeile mich. / Ich freue mich. / Ich merke mir etwas  etc.
> • Manche Verben kann man reflexiv oder nicht reflexiv benutzen.
> • nicht reflexiv: Ich kenne den Professor. / Ich vertraue dir nicht.
> • Ich ziehe das Kind an. / Ich ziehe dem Kind eine Jacke an.
> • reflexiv: Ich kenne mich (selbst). / Ich vertraue mir (selbst) nicht.
> • Ich ziehe mich (selbst) an. / Ich ziehe mir (selbst) eine Jacke an.
> • nicht reflexiv:
> • reflexiv:
> • Reflexivpron./ Akk.    Reflexivpron./ Dat. Akkusativobjekt
> • Ich setze
> • Du setzt
> • Er / Sie / Es setzt
> • Wir setzen
> • Ihr setzt
> • Sie setzen
> • mich.
> • dich.
> • sich.
> • uns.
> • euch.
> • sich.
> • Ich kaufe
> • Du kaufst
> • Er / Sie / Es kauft
> • Wir kaufen
> • Ihr kauft
> • Sie kaufen
> • mir
> • dir
> • sich
> • uns
> • euch
> • sich
> • ein E-Bike.
> • eine neue Hose.
> • einen Wintermantel.
> • Pommes.
> • eine Wohnung.
> • ein Boot.
> • Erinnert Max sich? ➔ auch möglich: Erinnert sich Max? ➔➔ Das Subjekt ist kein Pronomen.
> • Erinnerst du dich? ➔ nicht möglich: Erinnerst dich du?  ➔➔ Das Subjekt ist ein Pronomen.


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Subjekt Aktion Akk.Obj.
> • Ich wasche  mich. Ich wasche  mir    das Gesicht .
> • Akk. Obj.
> • Subjekt Aktion reflexiv - Akk. Subjekt Aktion reflexiv - Dat.


### ✍️ Übung 1
> 💡 **Beispiel:** Klaus hat Urlaub.   Klaus   freut   sich  , denn er hat Urlaub.

- **a)** Ich treffe Lisa morgen.
- **b)** Wir bekommen Besuch.
- **c)** Du bist wieder gesund.
- **d)** *(Carmen)* hat bald Ferien.
- **e)** Wir fahren nach Portugal.
- **f)** Ihr bekommt Geschenke.
- **g)** Du hast Geburtstag.
- **h)** Ich habe heute frei.
- **i)** Max geht ins Theater.

### ✍️ Übung 2
> 💡 **Beispiel:** Schämst du    dich ?

- **a)** Beeilt Lena _______?
- **b)** Freust du _______?
- **c)** Entschuldigt Maria _______?
- **d)** Wunderst du _______?
- **e)** Fürchtet ihr _______?
- **f)** Meldet ihr _______?
- **g)** *(Setzen)* wir _______?
- **h)** Entscheidet Murat _______?
- **i)** Ärgert Lisa _______?
- **j)** Stellst du _______ vor?
- **k)** Erinnert Karl _______?
- **l)** *(Ruhen)* wir _______ aus?

### ✍️ Übung 3
*Wählen Sie aus dem Wortkasten: Ich, wasche, das, Obst.*

> 💡 **Beispiel:** ein Tee - bestellen   Du   bestellst   dir     einen Tee  .

- **a)** das Auto — mieten
- **b)** die Zähne — putzen
- **c)** ein Fahrrad — wünschen
- **d)** eine Jacke — kaufen
- **e)** ein Kaffee — machen
- **f)** ein Bier — holen
- **g)** eine Suppe — kochen
- **h)** dein Platz — suchen
- **i)** die Haare — färben

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 13)</strong></summary>

> *Didaktische Lösungen für **3.3. Reflexive Verben**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `Carmen` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `Setzen` — Konjugiertes Verb <em>Setzen</em> (1p)
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `Ruhen` — Konjugiertes Verb <em>Ruhen</em> (1p)

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 13)**](#workbooks:A2:13) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-14"></a>
## 📄 Seite 14: 4.1. Verben mit Präpositionalobjekt

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Lektion 4
> • 4.1. Verben mit Präpositionalobjekt
> • Maria besucht ihren Freund. ➔ Akkusativobjekt
> • Paul hilft mir. ➔ Dativobjekt
> • Die Leute warten auf den Bus. ➔ Präpositionalobjekt
> • persönliches Subj.
> • achten / aufpassen auf (A)
> • anfangen / beginnen mit (D)
> • aufhören mit (D)
> • denken an (A)
> • nachdenken über (A)
> • fliehen / flüchten vor (D)
> • lachen über (A)
> • protestieren gegen (A)
> • rechnen mit (D)
> • sorgen für (A)
> • teilnehmen an (D)
> • warten auf (A)
> • unpersönliches Subj.
> • gehören zu (D)
> • passen zu (D)
> • riechen nach (D)
> • schmecken nach (D)
> • persönliches Subj. + Akk.Obj.
> • bitten dich um (A)
> • einladen dich zu (D)
> • fragen dich nach (D)
> • informieren dich über (A)
> • persönliches Subj. + Dat.Obj.
> • antworten dir auf (A)
> • danken dir für (A)
> • gratulieren dir zu (D)
> • helfen dir bei (D)
> • raten dir zu (D)
> • persönliches Subj./ reflexiv
> • sich ärgern über (A)
> • sich engagieren für (A)
> • sich erinnern an (A)
> • sich fürchten vor (D)
> • sich gewöhnen an (A)
> • sich interessieren für (A)
> • sich kümmern um (A)
> • sich sorgen um (A)
> • sich treffen mit (D)
> • sich verabreden mit (D)
> • sich verabschieden von (D)
> • sich verlassen auf (A)
> • sich wenden an (A)
> • sich wundern über (A)
> • zwei Präp.Obj.
> • diskutieren mit (D) über (A)
> • sich anmelden bei (D) für (A)
> • sich bedanken bei (D) für (A)
> • sich beschweren bei (D) über (A)
> • sich entschuldigen bei (D) für (A)
> • sich erkundigen bei (D) nach (D)
> • sich streiten mit (D) über (A)
> • sich unterhalten mit (D) über (A)
> • verschiedene Präp.Obj.
> • arbeiten an (D) / mit (D) / für (A)
> • sich bewerben um (A) / für (A)
> • sich entscheiden für (A) / gegen (A)
> • erzählen von (D) / über (A)
> • sich freuen auf (A) / über (A)
> • sprechen mit (D) über (A) / von (D)
> • sich vorbereiten auf (A) / für (A)


### ✍️ Übung 1
*Ergänzen Sie eine Präposition.*

- **a)** Viele Leute protestieren _______ die Regierung.
- **b)** Wir haben schon _______ der Arbeit begonnen.
- **c)** Carlo denkt immer nur _______ seine Freundin.
- **d)** Die Eltern sorgen _______ die Kinder.
- **e)** Rechnest du _______ einem Unwetter?
- **f)** Nimmst du _______ der Reise teil?
- **g)** *(Haben)* die Leute _______ deinen Witz gelacht?
- **h)** Achtest du _______ gesundes Essen?
- **i)** Passt die Farbe der Krawatte _______ dem Hemd?
- **j)** Schmeckt der Kuchen _______ Schokolade?
- **k)** Die Leute an der Küste fliehen _______ dem Sturm.
- **l)** Denkst du bitte _______ mein Angebot nach?

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 14)</strong></summary>

> *Didaktische Lösungen für **4.1. Verben mit Präpositionalobjekt**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `Habt` — Konjugiertes Verb <em>Habt</em> (3s)
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 14)**](#workbooks:A2:14) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-15"></a>
## 📄 Seite 15: 4.1. Verben mit Präpositionalobjekt — Vertiefung & Übungen

### ✍️ Übung 2
*Ergänzen Sie dir oder dich und eine Präposition.*

> 💡 **Beispiel:** Ich antworte dir   auf  deine Frage.

- **a)** Ich danke d_______ _______ deine Hilfe.
- **b)** Ich frage d_______ _______ dem Weg.
- **c)** Ich gratuliere d_______ _______ deinem Geburtstag.
- **d)** Ich helfe d_______ _______ der Arbeit.
- **e)** Ich lade d_______ _______ meiner Party ein.
- **f)** Ich informiere d_______ _______ den Termin.
- **g)** Ich bitte d_______ _______ deinen Rat.

### ✍️ Übung 3
*Ergänzen Sie ein Reflexivpronomen und eine Präposition.*

- **a)** Paul engagiert _______ _______ seine politischen Ziele.
- **b)** Wann trefft ihr _______ _______ eurer Tante?
- **c)** Kümmerst du _______ bitte _______ meine Katze?
- **d)** Interessiert ihr _______ nicht _______ Politik?
- **e)** Warum ärgert er _______ _______ den Professor?
- **f)** Ich erinnere _______ nicht _______ die Adresse.
- **g)** Musst du _______ jetzt _______ uns verabschieden?
- **h)** Bewirbst du _______ _______ eine neue Arbeit?
- **i)** Max möchte _______ gerne _______ Julia verabreden.

### ✍️ Übung 4
*Ergänzen Sie die Präpositionen.*

- **a)** Freust du dich _______ dein Geschenk?
- **b)** *(Freuen)* Sie sich _______ Ihren Urlaub?
- **c)** Streitet ihr euch oft _______ euren Eltern?
- **d)** Streitest du _______ Politik?
- **e)** Entschuldigt sich Max _______ seinen Fehler?
- **f)** Entschuldigst du dich _______ mir?
- **g)** Unterhältst du dich oft _______ deiner Kollegin?
- **h)** Unterhaltet ihr euch gern _______ Kinofilme?
- **i)** Erkundigt Max sich _______ den Preisen für Hotelzimmer?
- **j)** Erkundigst du dich _______ deinem Lehrer?
- **k)** Beschwerst du dich _______ der Deutschen Post?
- **l)** Beschwert ihr euch _______ den schlechten Service?
- **m)** Entscheidest du dich _______ oder _______ die Reise?

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 15)</strong></summary>

> *Didaktische Lösungen für **4.1. Verben mit Präpositionalobjekt — Vertiefung & Übungen**:*

#### Übung 2
- **a)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (de)
- **b)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (de)
- **c)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (de)
- **d)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (de)
- **e)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (de)
- **f)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (de)
- **g)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (de)

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung

#### Übung 4
- **a)** `—` — Grammatische Ergänzung
- **b)** `Freuen` — Konjugiertes Verb <em>Freuen</em> (3p)
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 15)**](#workbooks:A2:15) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-16"></a>
## 📄 Seite 16: 4.2. Pronominaladverbien

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 4.2. Pronominaladverbien
> • Präpositionalobjekt - Person / Wesen Präposition + Personalpronomen
> • Ärgerst du dich über deinen Freund? - Ja, ich ärgere mich über ihn.
> • Kümmerst du dich um die Katze? - Ja, ich kümmere mich um sie.
> • Präpositionalobjekt - Sache / Vorgang Pronominaladverb
> • Ärgerst du dich über die Verspätung? - Ja, ich ärgere mich darüber.
> • Hast du mit dieser Chance gerechnet? - Ja, ich habe        damit  gerechnet.
> • Bildung der Pronominaladverbien ➔ r:
> • z. B. an / auf / aus / über / um ➔➔ da-r-an / da-r-auf / da-r-aus / da-r-über / da-r-um
> • für / mit / nach / von / zu ➔➔ da-für / da-mit / da-nach / da-von / da-zu


### ✍️ Übung 1
> 💡 **Beispiel:** Hast du        über          den Witz gelacht? -  Natürlich habe ich       darüber       gelacht.

- **a)** Hast du dich _______ die schlechte Organisation beschwert?
- **b)** Hat Jonas sich _______ diese Arbeit interessiert?
- **c)** *(Haben)* Sie sich _______ die Geschenke gefreut?
- **d)** Habt ihr _______ die Verabredung gedacht?
- **e)** *(Haben)* Sie _______ dem Seminar teilgenommen?
- **f)** Hast du _______ der Adresse gefragt?
- **g)** Hat Laura sich _______ die Reservierung gekümmert?
- **h)** Hat Hatem _______ das Angebot nachgedacht?
- **i)** *(Haben)* die Leute sich _______ den Kurs angemeldet?
- **j)** Hast du dich _______ das Projekt engagiert?
- **k)** Habt ihr euch _______ den Termin erinnert?
- **l)** Hast du _______ Ruhe gebeten?
- **m)** Hat Julia sich _______ die Prüfung informiert?

### ✍️ Übung 2
*Jan - warten  Hast du       auf       Jan gewartet? - Nein, ich habe nicht       auf ihn       gewartet.*

> 💡 **Beispiel:** e:

- **a)** der Lärm — sich wundern
- **b)** die Arbeit — sich konzentrieren
- **c)** die Gäste — sich verabschieden
- **d)** der Strafzettel — protestieren
- **e)** dein Onkel — sich unterhalten
- **f)** die Prüfung — sich vorbereiten
- **g)** die Kinder — aufpassen
- **h)** die Einladung — sich freuen
- **i)** die Kollegin — sprechen
- **j)** unsere Probleme — sprechen
- **k)** das Geschenk — sich bedanken
- **l)** dein Onkel — sich bedanken
- **m)** der Fehler — sich entschuldigen
- **n)** der Freund — sich entschuldigen
- **o)** die Verabredung — sich erinnern
- **p)** deine Schulfreundin — sich erinnern

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 16)</strong></summary>

> *Didaktische Lösungen für **4.2. Pronominaladverbien**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `Haben` — Konjugiertes Verb <em>Haben</em> (3p)
- **d)** `—` — Grammatische Ergänzung
- **e)** `Haben` — Konjugiertes Verb <em>Haben</em> (3p)
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `Habt` — Konjugiertes Verb <em>Habt</em> (3s)
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung
- **o)** `—` — Grammatische Ergänzung
- **p)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 16)**](#workbooks:A2:16) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-17"></a>
## 📄 Seite 17: 4.2. Pronominaladverbien — Vertiefung & Übungen

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Person Präposition + Fragewort
> • Ich ärgere mich über meine Nachbarin. - Über wen   ärgerst du dich?
> • Sache oder Vorgang Pronominaladverb (als Fragewort)
> • Ich ärgere mich über die Verspätung. - Worüber    ärgerst du dich?


### ✍️ Übung 3
*Wählen Sie aus dem Wortkasten: ihr, warten, unsere, Tante, Auf, wen, habt, ihr, gewartet?, Auf, unsere, Tante.*

> 💡 **Beispiel:** e:  du - sich informieren - die Preise    Wor  über   hast du dich informiert? -   Über   die Preise.

- **a)** du — sich erkundigen — Lisas Adresse
- **b)** Max — sich interessieren — Politik
- **c)** Lena — sich bedanken — Theo
- **d)** ihr — sich beschweren — der Lärm
- **e)** du — sich entschuldigen — mein Fehler
- **f)** ihr — sich unterhalten — unsere Freunde
- **g)** die Studenten — sich vorbereiten — die Prüfung
- **h)** Julia — sprechen — ihr Onkel
- **i)** ihr — sich erinnern — unser Urlaub in Spanien
- **j)** du — sich konzentrieren — meine Arbeit
- **k)** Paul — sich verabschieden — seine Gäste
- **l)** der Autofahrer — protestieren — der Strafzettel

### ✍️ Übung 4
*Wählen Sie aus dem Wortkasten: Ich, habe, mich, darüber, gefreut, dass, Eva, mich, angerufen, hat.*

> 💡 **Beispiel:** Worüber hast du dich gefreut? - Eva hat mich angerufen.

- **a)** _______ hast du nicht gerechnet? — Heute regnet es.
- **b)** _______ hast du dich entschuldigt? — Ich habe mich verspätet.
- **c)** _______ hast du Max gedankt? — Er hat mir sehr geholfen.
- **d)** _______ hast du Lena gebeten? — Sie leiht mir ihr Fahrrad.
- **e)** _______ kannst du dich nicht gewöhnen? — Hier ist es im Winter so kalt.
- **f)** _______ hast du dich gefürchtet? — Der Hund beißt mich.
- **g)** _______ hast du dich gewundert? — Clara kann nicht schwimmen.
- **h)** _______ hast du dich beschwert? — Meine Suppe war nicht mehr heiß.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 17)</strong></summary>

> *Didaktische Lösungen für **4.2. Pronominaladverbien — Vertiefung & Übungen**:*

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

#### Übung 4
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 17)**](#workbooks:A2:17) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-18"></a>
## 📄 Seite 18: 4.3. Nebensätze - konditional

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 4.3. Nebensätze - konditional
> • Hast du morgen Zeit oder nicht? ➔  Wir können uns treffen. - Vielleicht habe ich Zeit.
> • Gut, wenn du Zeit hast, (dann) können wir uns treffen.
> • Wenn du keine Zeit hast, (dann) können wir uns nicht treffen.
> • Bedingung: Zeit haben  ➔  Option: treffen   ➔   Wenn ..., (dann) ...
> • Wenn / Falls es morgen regnet, (dann) gehen wir nicht in den Zoo.
> • Wenn / Falls es morgen nicht regnet, (dann) gehen wir in den Zoo.
> • Konditionale NS kann man mit wenn oder falls einleiten.


### ✍️ Übung 1
*Wählen Sie aus dem Wortkasten: Du, musst, dich, in, die, erste, Reihe, setzen, wenn, du, schlecht, siehst*

> 💡 **Beispiel:** Siehst du schlecht. Dann setz dich in die erste Reihe!

- **a)** Brauchst du Urlaub? Dann geh doch zum Chef!
- **b)** Kennst du den Weg nicht? Dann frag doch jemanden!
- **c)** Hast du Probleme mit dieser Aufgabe? Dann bitte doch die Lehrerin um Hilfe!
- **d)** Suchst du eine Wohnung? Dann gib doch eine Anzeige auf!
- **e)** Willst du in den Bergen wandern? Dann kauf dir doch gute Schuhe!
- **f)** Ist dir langweilig? Dann lies doch ein Buch!
- **g)** Brauchst du Hilfe? Dann ruf mich doch an!
- **h)** Ist dir kalt? Dann mach doch das Fenster zu!
- **i)** Glaubst du mir nicht? Dann frag doch Peter!
- **j)** Hast du Hunger? Dann iss doch etwas!

### ✍️ Übung 2
*Wählen Sie aus dem Wortkasten: Wenn, Paul, anruft, dann, informiere, ich, dich.*

> 💡 **Beispiel:** Vielleicht ruft Paul an. > Ich informiere dich.

- **a)** Wahrscheinlich schaffe ich die Prüfung. > Ich mache eine Party.
- **b)** Vielleicht hat Max gelogen. > Ich vertraue ihm nie wieder.
- **c)** Vielleicht kann ich die Arbeit vor 18:00 Uhr erledigen. > Ich kann ins Kino gehen.
- **d)** Wahrscheinlich kommt Clara wieder zu spät. > Ich werde richtig sauer.
- **e)** Vielleicht hat Lena die Nachricht nicht bekommen. > Sie kann natürlich nicht antworten.
- **f)** Wahrscheinlich fängt Karl eine Arbeit an. > Er kann nächsten Monat nicht verreisen.
- **g)** Vielleicht möchte Theo gesünder leben. > Er muss mehr Obst und Gemüse essen.

### ✍️ Übung 3
*Bilden Sie konditionale Nebensätze.*

> 💡 **Beispiel:** Du musst mir helfen. > Sonst / Andernfalls schaffe ich die Arbeit nicht rechtzeitig.

- **a)** Ich muss bald buchen. Sonst bekomme ich kein Hotelzimmer mehr.
- **b)** Wir müssen uns beeilen. Sonst kommen wir zu spät.
- **c)** Ihr dürft nicht alles glauben. Sonst halten euch die Leute für naiv.
- **d)** Theo muss sparen. Andernfalls kann er nicht in Urlaub fahren.
- **e)** Du musst dich wärmer anziehen. Sonst erkältest du dich.
- **f)** Ich muss Geld abheben. Andernfalls kann ich die Rechnung nicht bezahlen.
- **g)** Du musst eine stärkere Sonnencreme benutzen. Sonst bekommst du einen Sonnenbrand.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 18)</strong></summary>

> *Didaktische Lösungen für **4.3. Nebensätze - konditional**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 18)**](#workbooks:A2:18) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-19"></a>
## 📄 Seite 19: 5.1. Konjunktiv II (würde / hätte / wäre / Modalverben)

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Lektion 5
> • 5.1. Konjunktiv II (würde / hätte / wäre / Modalverben)
> • ich würde
> • du würdest
> • er / sie / es würde
> • wir würden
> • ihr würdet
> • sie / Sie würden
> • ich hätte
> • du hättest
> • er / sie / es  hätte
> • wir hätten
> • ihr hättet
> • sie / Sie hätten
> • ich wäre
> • du wärest
> • er / sie / es wäre
> • wir wären
> • ihr wäret
> • sie / Sie wären


### ✍️ Übung 1
*Wählen Sie aus dem Wortkasten: Präsens:, Wenn, ich, Urlaub, bekommen, würde, könnte, ich, dich, besuchen.*

> 💡 **Beispiel:** müssen - ich muss -  ich müsste

- **a)** *(dürfen)*  — ich darf — _______
- **b)** *(können)*  — ich kann — _______
- **c)** *(mögen)*  — ich mag — _______
- **d)** *(wollen)*  — ich will — _______
- **e)** *(sollen)*  — ich soll — _______
- **f)** *(haben)*  — ich habe — _______
- **g)** sein — ich bin — _______
- **h)** *(werden)*  — ich werde — _______

### ✍️ Übung 2
*Wählen Sie aus dem Wortkasten: Rufen, Sie, mich, an?, Würden, Sie, mich, bitte, anrufen?*

> 💡 **Beispiel:** Anja fährt zu schnell. Es       wäre       besser, wenn sie nicht       so             schnell       fahren würde      .

- **a)** Boris spricht zu leise.
- **b)** Christa trinkt zu wenig.
- **c)** Dieter schläft zu kurz.
- **d)** Eva treibt zu wenig Sport.
- **e)** Franz gibt zu viel Geld aus.
- **f)** Clara liest zu wenig.
- **g)** Martin erzählt zu viel.
- **h)** Inge lügt zu oft.
- **i)** Amira denkt zu viel nach.

### ✍️ Übung 3
> 💡 **Beispiel:** Grüßt du Paul von mir.   Könntest   du Paul von mir grüßen?

- **a)** Bringt ihr uns zum Bahnhof?
- **b)** Trägst du die Tasche?
- **c)** Nimmst du mich mit?
- **d)** Lädst du mich ein?
- **e)** Liest du die Aufgabe vor?
- **f)** Begleitet ihr mich nach Hause?
- **g)** Tust du mir einen Gefallen?
- **h)** Schickt ihr mir eine Karte?
- **i)** Stellst du die Blumen in die Vase?
- **j)** *(Machen)* Sie den Fernseher aus?
- **k)** *(Rufen)* Sie noch einmal an?
- **l)** Gibst du mir den Kugelschreiber?
- **m)** Leihst du mir dein Auto?
- **n)** Schließt du die Tür?

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 19)</strong></summary>

> *Didaktische Lösungen für **5.1. Konjunktiv II (würde / hätte / wäre / Modalverben)**:*

#### Übung 1
- **a)** `dürfe` — Konjugiertes Verb <em>dürfe</em> (1s)
- **b)** `könne` — Konjugiertes Verb <em>könne</em> (1s)
- **c)** `möge` — Konjugiertes Verb <em>möge</em> (1s)
- **d)** `wolle` — Konjugiertes Verb <em>wolle</em> (1s)
- **e)** `solle` — Konjugiertes Verb <em>solle</em> (1s)
- **f)** `habe` — Konjugiertes Verb <em>habe</em> (1s)
- **g)** `—` — Grammatische Ergänzung
- **h)** `werde` — Konjugiertes Verb <em>werde</em> (1s)

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `Machen` — Grammatische Ergänzung
- **k)** `Rufen` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 19)**](#workbooks:A2:19) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-20"></a>
## 📄 Seite 20: 5.2. Präpositionen mit Dativ und Akkusativ

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 5.2. Präpositionen mit Dativ und Akkusativ
> • Präposition Frage Kasus Beispiel
> • an
> • wo? Dat. (Position) an der Tafel
> • wohin? Akk. (Aktion) an die Tafel
> • wann? Dat. (Zeit) am Samstag
> • auf wo? Dat. (Position) auf dem Tisch
> • wohin? Akk. (Aktion) auf den Tisch
> • in
> • wo? Dat. (Position) in der Pension
> • wohin? Akk. (Aktion) in die Pension
> • wann? Dat. (Zeit) im Winter
> • hinter wo? Dat. (Position) hinter dem Hotel
> • wohin? Akk. (Aktion) hinter das Hotel
> • vor
> • wo? Dat. (Position) vor dem Krankenhaus
> • wohin? Akk. (Aktion) vor das Krankenhaus
> • wann? Dat. (Zeit) vor einer Woche
> • über wo? Dat. (Position) über dem Tisch
> • wohin? Akk. (Aktion) über den Tisch
> • unter wo? Dat. (Position) unter dem Sofa
> • wohin? Akk. (Aktion) unter das Sofa
> • neben wo? Dat. (Position) neben dem Haus
> • wohin? Akk. (Aktion) neben das Haus
> • zwischen
> • wo? Dat. (Position) zwischen mir und der Kollegin
> • wohin? Akk. (Aktion) zwischen mich und die Kollegin
> • wann? Dat. (Zeit) zwischen dem 20. Mai und dem 1. Juni
> • Manchmal sind diese Präpositionen mit dem Artikel verbunden:
> • am ➔ an dem ans ➔ an das im ➔ in dem ins ➔ in das aufs ➔ auf das vors ➔ vor das


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Herr Berg geht ins Büro.
> • Wo arbeitet Herr Berg? (Position)
> • Herr Berg arbeitet im Büro.


### ✍️ Übung 1: Übung 2
*wohin? wo?*

- **a)** Gehst du in _______ Schule?
- **b)** Gehst du in _______ Firma?
- **c)** Gehst du in _______ Dom?
- **d)** Gehst du in _______ Kantine?
- **e)** Gehst du in_______ Café?
- **f)** Gehst du in _______ Garage?
- **a)** Ist er in _______ Werkstatt?
- **b)** Ist sie in _______ Universität?
- **c)** Ist er in _______ Küche?
- **d)** Ist sie in_______ Wohnzimmer?
- **e)** Ist er in_______ Badezimmer?
- **f)** Ist sie in_______ Keller?

### ✍️ Übung 3
*Ergänzen Sie die Endungen.*

- **a)** Sie wartet an d_______ Bushaltestelle.
- **b)** Gehst du heute in d_______ Disko?
- **c)** Die Lampe hängt über d_______ Schreibtisch.
- **d)** Schreib bitte die Adresse auf d_______ Karte!
- **e)** Frau Holm bleibt zwei Tage in d_______ Schweiz.
- **f)** Das Auto steht vor d_______ Garage.
- **g)** Fährst du morgen an d_______ Bodensee?
- **h)** Der Garten ist hinter d_______ Haus.
- **i)** Ich sitze zwischen Max und mein_______ Vater.
- **j)** Stell die Gläser auf d_______ Tisch!
- **k)** Die Schuhe stehen unter d_______ Sofa.
- **l)** Der Bus hält vor d_______ Rathaus.
- **m)** Maria sitzt hinter ihr_______ Schreibtisch.
- **n)** Du stehst neben m_______.
- **o)** Sie nimmt das Buch in d_______ Hand.
- **p)** Sie stellt die Stiefel vor d_______ Tür.
- **q)** Sie will über d_______ Straße gehen.
- **r)** Lebst du in d_______ Stadt? — Nein, auf d_______ Land.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 20)</strong></summary>

> *Didaktische Lösungen für **5.2. Präpositionen mit Dativ und Akkusativ**:*

#### Übung 1: Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `st` — Subjekt (2s) &rarr; Endung <code>-st</code> (inst)
- **f)** `—` — Grammatische Ergänzung
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `en` — Subjekt (3p) &rarr; Endung <code>-en</code> (inen)
- **e)** `t` — Subjekt (3s) &rarr; Endung <code>-t</code> (int)
- **f)** `en` — Subjekt (3p) &rarr; Endung <code>-en</code> (inen)

#### Übung 3
- **a)** `en` — Subjekt (3p) &rarr; Endung <code>-en</code> (den)
- **b)** `est` — Subjekt (2s) &rarr; Endung <code>-est</code> (dest)
- **c)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **d)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **e)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **f)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **g)** `est` — Subjekt (2s) &rarr; Endung <code>-est</code> (dest)
- **h)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **i)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (meine)
- **j)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **k)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **l)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **m)** `t` — Subjekt (3s) &rarr; Endung <code>-t</code> (ihrt)
- **n)** `st` — Subjekt (2s) &rarr; Endung <code>-st</code> (mst)
- **o)** `en` — Subjekt (3p) &rarr; Endung <code>-en</code> (den)
- **p)** `en` — Subjekt (3p) &rarr; Endung <code>-en</code> (den)
- **q)** `en` — Subjekt (3p) &rarr; Endung <code>-en</code> (den)
- **r)** `est` — Subjekt (2s) &rarr; Endung <code>-est</code> (dest)

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 20)**](#workbooks:A2:20) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-21"></a>
## 📄 Seite 21: 5.2. Präpositionen mit Dativ und Akkusativ — Vertiefung & Übungen

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • die Jahreszeiten
> • der Frühling / das Frühjahr - der Sommer - der Herbst - der Winter
> • die Monate
> • der Januar - der Februar - der März - der April - der Mai - der Juni - der Juli
> • der August - der September - der Oktober - der November - der Dezember
> • die Wochentage
> • der Sonntag - der Montag - der Dienstag - der Mittwoch - der Donnerstag - der Freitag - der Samstag
> • die Tageszeiten
> • der Morgen - der Vormittag - der Mittag - der Nachmittag - der Abend - die Nacht
> • schon ←➔ erst: früh oder früher als gedacht ➔ schon / spät oder später als gedacht ➔ erst
> • Bist du schon heute fertig? ➔ Nein, ich werde erst morgen fertig.


### ✍️ Übung 4
*Ergänzen Sie die Endungen.*

- **a)** Was steht i_______ Brief?
- **b)** Der Hund sitzt unter d_______ Tisch.
- **c)** Sie trägt den Hund vor d_______ Tür.
- **d)** Paul möchte morgen auf__ Land fahren.
- **e)** Setz dich bitte neben dein_______ Onkel!
- **f)** Maria sitzt zwischen Peter und m_______.
- **g)** Er will in ein_______ Monat nach Rom reisen.
- **h)** Wir wollten morgen in d_______ Berge fahren.
- **i)** Kannst du mir a_______ Wochenende helfen?
- **j)** Ich lebe nicht in d_______ Stadt, sondern auf d_______ Dorf.

### ✍️ Übung 5
> 💡 **Beispiel:** Sie geht an     die    Kasse. ( wohin? ) -  Sie steht an    der    Kasse. ( wo? )

- **a)** Das Buch liegt auf _______ Schreibtisch.
- **b)** Stell den Wagen bitte vor _______ Garage!
- **c)** Meine Schwester lebt in _______ Schweiz.
- **d)** Maria sitzt auf _______ Sofa.
- **e)** Sandra geht schnell über _______ Straße.
- **f)** Eva sitzt zwischen m_______ und ihr_______ Tante.
- **g)** Steck den Brief in _______ Tasche!
- **h)** Schreib die Adresse auf _______ Zettel!
- **i)** Ich gehe i_______ / in _______ Park spazieren. (!)
- **j)** Du läufst auf _______ / auf _______ Straße. (!)

### ✍️ Übung 6
*Wählen Sie aus dem Wortkasten: Das, Auto, fährt, auf, der, Autobahn., wo?*

> 💡 **Beispiel:** Nachmittag - Berlin  Fährst du schon am Abend nach Berlin? - Nein, erst am Vormittag.

- **a)** Mai — Spanien
- **b)** Dienstag — Köln
- **c)** ein Monat — Ungarn
- **d)** Abend — Stuttgart
- **e)** Wochenende — Frankfurt
- **f)** Winter — Griechenland
- **g)** Freitag — Dortmund
- **h)** Nacht — Nürnberg
- **i)** Sommer — Schweiz
- **j)** *(Morgen)*  — Klaus
- **k)** Juli — Meer
- **l)** Mittwoch — Türkei

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 21)</strong></summary>

> *Didaktische Lösungen für **5.2. Präpositionen mit Dativ und Akkusativ — Vertiefung & Übungen**:*

#### Übung 4
- **a)** `t` — Subjekt (3s) &rarr; Endung <code>-t</code> (it)
- **b)** `et` — Subjekt (3s) &rarr; Endung <code>-et</code> (det)
- **c)** `en` — Subjekt (3p) &rarr; Endung <code>-en</code> (den)
- **d)** `—` — Grammatische Ergänzung
- **e)** `t` — Subjekt (3s) &rarr; Endung <code>-t</code> (deint)
- **f)** `t` — Subjekt (3s) &rarr; Endung <code>-t</code> (mt)
- **g)** `t` — Subjekt (3s) &rarr; Endung <code>-t</code> (eint)
- **h)** `en` — Subjekt (1p) &rarr; Endung <code>-en</code> (den)
- **i)** `st` — Subjekt (2s) &rarr; Endung <code>-st</code> (ast)
- **j)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (de)

#### Übung 5
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `t` — Subjekt (3s) &rarr; Endung <code>-t</code> (mt)
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `e` — Subjekt (1s) &rarr; Endung <code>-e</code> (ie)
- **j)** `—` — Grammatische Ergänzung

#### Übung 6
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `Morgen` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 21)**](#workbooks:A2:21) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-22"></a>
## 📄 Seite 22: 5.3. Ähnliche Verben

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 5.3. Ähnliche Verben
> • Aktion  -  Position
> • Aktionsverb: wohin? (+ Akk.) Positionsverb: wo? (+ Dat.)
> • setzen (sich)
> • Ich setze den Hund in den Korb.
> • Ich setze mich auf meinen Platz.
> • stellen (sich)
> • Ich stelle die Flasche auf den Tisch.
> • Ich stelle mich in die Reihe.
> • legen (sich)
> • Ich lege den Wein in den Kühlschrank.
> • Ich lege mich auf das Sofa.
> • hängen
> • Ich hänge den Mantel in die Garderobe.
> • stecken
> • Ich stecke den Brief ins Kuvert.
> • sitzen
> • Der Hund sitzt im Korb.
> • stehen
> • Die Flasche steht auf dem Tisch.
> • liegen
> • Der Wein liegt im Kühlschrank.
> • hängen
> • Der Mantel hängt in der Garderobe.
> • stecken
> • Der Brief steckt im Kuvert.
> • Übung
> • Beispiel:  legen ←➔ liegen ➔ Max - in - Krankenhaus   Max liegt im Krankenhaus.
> • stellen
> • stehen
> • a) Ich - Glas - auf - Tisch
> • b) Glas - neben - Flasche
> • c) Du - Auto - vor - Garage
> • d) Paul - vor - Tür
> • e) Viele Leute - an - Kinokasse
> • f) Maria - Roller - vor - Haus
> • g) Wir - an - Bushaltestelle
> • h) Er - zwischen - seine Eltern
> • legen
> • liegen
> • a) Brief - auf - Schreibtisch
> • b) Julia - auf - Sofa
> • c) Maria - Buch - in - Regal
> • d) Er - Pass - neben - Ticket
> • e) Sie - Hut - in - Schrank
> • g) Ich - in - Bett
> • h) Du - Mappe - auf - Tisch
> • setzen
> • sitzen
> • a) Ich - an - Fenster
> • b) Julia - Kind - auf - Bett
> • c) Du - Katze - auf - Sofa
> • d) Wir - in - Bus
> • e) Paul - sein Sohn - in - Auto
> • f) Ich - zwischen - Onkel - Tante
> • g) Eva - Brille - auf - Nase
> • h) Du - vor - Fernseher
> • hängen
> • a) Apfel - an - Baum
> • b) Ich - Uhr - an - Wand
> • c) Maria - Poster - an - Tür
> • d) Handtuch - neben - Badewanne
> • e) Paul - Hose - in - Schrank
> • f) Hut - in - Garderobe
> • g) Er - Jacke - über - Stuhl
> • h) Lampe - über - Tisch
> • stecken
> • a) Schlüssel - in - Schloss
> • b) Er - Geld - in - Tasche
> • c) Füße - in - Schuhe
> • d) Bleistift - in  - Jackentasche
> • e) Ich - Foto - zwischen - Briefe
> • f) Wir - Brief - in - Postkasten
> • g) Maria - Nadel - an - Jacke
> • h) Zettel - hinter - Spiegel


<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 22)</strong></summary>

> *Didaktische Lösungen für **5.3. Ähnliche Verben**:*

*(Auf dieser Seite befinden sich Einführungstexte, Inhaltsverzeichnisse oder freie Textübungen.)*

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 22)**](#workbooks:A2:22) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-23"></a>
## 📄 Seite 23: 6.1. Präteritum (Imperfekt)

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Lektion 6
> • 6.1. Präteritum (Imperfekt)
> • Präteritum - schwache Verben (regelmäßig)
> • Infinitiv
> • Präteritum
> • ich kauf-t-e
> • du kauf-t-est
> • er / sie / es kauf-t-e
> • wir kauf-t-en
> • ihr kauf-t-et
> • sie / Sie kauf-t-en
> • ich arbeit-et-e
> • du arbeit-et-est
> • er / sie / es arbeit-et-e
> • wir arbeit-et-en
> • ihr arbeit-et-et
> • sie / Sie arbeit-et-en
> • Verben mit Stamm auf -t, -d, chn, ffn, gn ➔ Präteritum mit et
> • ich antwortete, wir redeten, er rechnete, ich öffnete, es regnete
> • Achtung: Immer schwach sind Verben auf  -eln, -ern, -igen, -ieren.
> • ich sammelte, ich änderte, ich besichtigte, ich telefonierte


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • kauf-en
> • kauf-t-e
> • arbeit-en
> • arbeit-et-e


### ✍️ Übung 1
> 💡 **Beispiel:** Wann kauft Max den Wagen? -  Er kauf  t  e ihn gestern.

- **a)** Wann fragt Theo die Lehrerin?
- **b)** Wann holst du die Tickets?
- **c)** Wann kündigt Yasmin den Vertrag?
- **d)** Wann repariert Murat das Fahrrad?
- **e)** Wann liefert man die Maschinen?
- **f)** Wann putzt du die Fenster?
- **g)** Wann ändert Julia den PIN — Code?
- **h)** Wann reservierst du den Tisch?
- **i)** Wann lernt ihr die starken Verben?
- **j)** Wann besuchst du Paul?
- **k)** Wann bucht ihr die Reise?
- **l)** Wann wechselt Lena das Geld?
- **m)** Wann besichtigt ihr die Burg?
- **n)** Wann mietest du das Auto?
- **o)** Wann gratulierst du deinem Großvater?
- **p)** Wann antwortet ihr der Kollegin?
- **q)** Wann entschuldigst du dich?
- **r)** Wann informiert ihr euch?

### ✍️ Übung 2
> 💡 **Beispiel:** ich - im Supermarkt -  einkaufen    Ich kauf  t  e im Supermarkt ein.

- **a)** du — den Keller — aufräumen
- **b)** ich — die Pakete — abholen
- **c)** Max — die Tür — zumachen
- **d)** du — das Angebot — ablehnen
- **e)** ihr — den Kredit — zurückzahlen
- **f)** wir — das Formular — ausfüllen
- **g)** ihr — das Gerät — ausschalten
- **h)** wir — alle Fenster — zumachen
- **i)** ich — die richtige Antwort — ankreuzen
- **j)** Jana — den Koffer — auspacken
- **k)** du — eine Kerze — anzünden
- **l)** Paul — den Termin — absagen

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 23)</strong></summary>

> *Didaktische Lösungen für **6.1. Präteritum (Imperfekt)**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung
- **o)** `—` — Grammatische Ergänzung
- **p)** `—` — Grammatische Ergänzung
- **q)** `—` — Grammatische Ergänzung
- **r)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 23)**](#workbooks:A2:23) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-24"></a>
## 📄 Seite 24: 6.1. Präteritum (Imperfekt) — Vertiefung & Übungen

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Präteritum - starke Verben (unregelmäßig)
> • Infinitiv
> • Präteritum
> • ich trank
> • du trank-st
> • er / sie / es trank
> • wir trank-en
> • ihr trank-t
> • sie / Sie trank-en


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Infinitiv
> • Präteritum
> • ich brach-t-e
> • du brach-t-est
> • er / sie / es brach-t-e
> • wir brach-t-en
> • ihr brach-t-et
> • sie / Sie brach-t-en
> • bringen - brachte
> • denken - dachte
> • wissen - wusste
> • brennen - brannte
> • kennen - kannte
> • nennen - nannte
> • rennen - rannte
> • senden - sandte
> • wenden - wandte


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • trink-en
> • trank
> • bring-en
> • brach-t-e


### ✍️ Übung 3
*Bilden Sie einen Satz im Präteritum.*

> 💡 **Beispiel:** ein Brief - schreiben  Ich   schrieb   einen Brief.

- **a)** kein Parkplatz — finden
- **b)** zur Bushaltestelle — laufen
- **c)** eine Cola — nehmen
- **d)** mit Paul — sprechen
- **e)** ein Stück Kuchen — essen
- **f)** nach Moskau — fliegen
- **g)** zu Hause — bleiben
- **h)** ins Kino — gehen
- **i)** meine Freunde — einladen
- **j)** in den Zug — einsteigen
- **k)** um 6.30 Uhr — aufstehen
- **l)** viel Geld — ausgeben
- **m)** *(letzten)* Sonntag — zurückkommen
- **n)** die Kollegin — anrufen
- **o)** nach Köln — mitfahren
- **p)** auf dem Sofa — einschlafen

### ✍️ Übung 4
> 💡 **Beispiel:** Paul bringt Blumen mit.  Paul   brachte   Blumen mit.

- **a)** Lena weiß die Antwort.
- **b)** Ich kenne den Professor nicht
- **c)** Der Dieb rennt mit der Tasche weg.
- **d)** Murat denkt oft an seine Eltern.
- **e)** Julia nennt ihre Adresse.
- **f)** Ich verbrenne den Brief.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 24)</strong></summary>

> *Didaktische Lösungen für **6.1. Präteritum (Imperfekt) — Vertiefung & Übungen**:*

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `letzten` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung
- **o)** `—` — Grammatische Ergänzung
- **p)** `—` — Grammatische Ergänzung

#### Übung 4
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 24)**](#workbooks:A2:24) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-25"></a>
## 📄 Seite 25: 6.2. Relativsätze

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 6.2. Relativsätze
> • Relativsätze mit Relativpronomen
> • Relativpronomen Verb am ENDE
> • Bring mir bitte den Brief, d e r auf dem Schreibtisch l i e g t.
> • Relativsätze können ein Nomen genauer erklären. ➔  Attribut
> • Kennst du den Herrn? - Welchen Herrn?
> • Nominativ Er sitzt dort drüben. ➔ Kennst du den Herrn, der dort drüben sitzt?
> • Akkusativ Lena hat ihn gegrüßt. ➔ Kennst du den Herrn, den Lena gegrüßt hat?
> • Dativ Paul hat ihm gratuliert. ➔ Kennst du den Herrn, dem Paul gratuliert hat?


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • ______ so gute Kritiken bekommen hat?
> • ______ dir Max gegeben hat?
> • b) Er hat ein Auto gekauft,
> • ______ schon neun Jahre alt ist.
> • ______ einen Elektromotor hat.
> • c) Wo hast du den Schal,
> • ______ Paul dir geschenkt hat?
> • ______ mir so gut gefallen hat?
> • d) Wie schmecken dir die Kekse,
> • ______ ich gebacken habe?
> • ______ aus der teuren Bäckerei sind?
> • e) Das ist der Zug,
> • ______ aus Hamburg kommt.
> • ______ ich nehmen muss.
> • f) Er spricht mit den Leuten,
> • ______ den Unfall gesehen haben.
> • ______ der Film nicht gefallen hat.
> • g) Kennst du die Frau,
> • ______ im III. Stock wohnt?
> • ______ dieser Schirm gehört?
> • h) Wo sind die Touristen,
> • ______ das Gepäck hier gehört?
> • ______ heute abreisen?
> • Am Bahnsteig standen viele  Leute . Sie warteten auf den Zug.
> • Am Bahnsteig standen viele Leute, die auf den Zug warteten.
> • Hauptsatz , Relativsatz
> • Viele Leute  standen am Bahnsteig. Sie warteten auf den Zug.
> • Viele Leute , die auf den Zug warteten, standen am Bahnsteig.
> • Hauptsatz (Teil 1)  , R e l a t i v s a t z , Hauptsatz (Teil 2)


### ✍️ Übung 1
> 💡 **Beispiel:** Wann kommt der Herr,     der    uns angerufen hat?  //  den  wir angerufen haben?

- **a)** Wie findest du den Roman,

### ✍️ Übung 2
*Wählen Sie aus dem Wortkasten: Das, Obst, das, du, im, Supermarkt, gekauft, hast, musst, du, auf, jeden, Fall, waschen.*

> 💡 **Beispiel:** Das Obst musst du auf jeden Fall waschen. Du hast es im Supermarkt gekauft.

- **a)** Der Zug steht auf Gleis 5. Er fährt nach München.
- **b)** Die Künstlerin ist sehr berühmt. Sie hat das Porträt des Präsidenten gemalt.
- **c)** Die Gäste beschweren sich. Das Essen hat ihnen nicht geschmeckt.
- **d)** Die Touristen kommen aus Prag. Ich habe sie auf dem Rathausplatz getroffen.
- **e)** Der Film war spannend. Ich habe ihn gestern gesehen.
- **f)** *(Den)* Salat bringt uns der Ober erst jetzt. Ich habe ihn vor einer halben Stunde bestellt.
- **g)** Die Leute kommen um 16.00 Uhr an. Ich will ihnen die Stadt zeigen.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 25)</strong></summary>

> *Didaktische Lösungen für **6.2. Relativsätze**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `Den` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 25)**](#workbooks:A2:25) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-26"></a>
## 📄 Seite 26: 6.2. Relativsätze — Vertiefung & Übungen

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Relativsätze mit Präposition + Relativpronomen
> • Paul ist ein Mensch, auf den du dich verlassen kannst.
> • Das ist ein Hund, vor dem man sich fürchten muss.
> • Das ist ein Problem, über das ich mit dir sprechen möchte.
> • Relativsätze mit Verben + Präposition  ➔ Präposition + Relativpronomen


### ✍️ Übung 3
> 💡 **Beispiel:** Wann besucht dich dein Onkel,        von dem          du mir schon so viel erzählt hast?

- **a)** Wie heißt die Kollegin, _______ du dich gerade unterhalten hast?
- **b)** Was sind das für Leute, _______ du dich verabschiedet hast?
- **c)** Lena ist wirklich ein Mädchen, _______ man sich verlassen kann.
- **d)** Wem gehört die Katze, _______ du dich kümmern musst?
- **e)** Mein Cousin ist ein Mensch, _______ man nur streiten kann.
- **f)** Mein Nachbar hat einen Hund, _______ ich mich fürchte.
- **g)** Auf diesem Foto siehst du meinen Opa, _______ ich mich kaum erinnern kann.
- **h)** Von wem sind die Blumen, _______ du dich so gefreut hast?
- **i)** Wann macht Clara die Reise, _______ sie immer spricht?
- **j)** Wann beginnt der Kurs, _______ du dich angemeldet hast?
- **k)** Ist das die Kollegin, _______ du dich sehr geärgert hast?
- **l)** Ist Politik ein Thema, _______ er sich interessiert?
- **m)** Wie war die Konferenz, _______ ihr teilgenommen habt?
- **n)** Was war das für eine Geschichte, _______ ihr so gelacht habt?
- **o)** Wann kommt der Bus,  _______ du wartest?
- **p)** Hat Thomas einen Fehler gemacht, _______ er sich entschuldigen muss?

### ✍️ Übung 4
*Wählen Sie aus dem Wortkasten: Professorin, Supermarkt, Kamera, Schuhverkäufer, Giraffe, Uhr, Ärztin, Schauspieler, Sofa*

> 💡 **Beispiel:** Ein Supermarkt    ist ein Gebäude,   in dem    man Lebensmittel einkaufen kann.

- **a)** _______ ist ein Mann, _______ man im Theater oder im Kino sehen kann.
- **b)** _______ ist ein Tier, _______ einen sehr langen Hals hat.
- **c)** _______ ist eine Frau, _______ man gehen kann, wenn man krank ist.
- **d)** _______ ist ein Möbel, _______ man sitzen oder liegen kann.
- **e)** _______ ist ein Gerät, _______ man die Zeit messen kann.
- **f)** _______ ist eine Frau, _______ an der Universität unterrichtet.
- **g)** _______ ist ein Mann, _______ Schuhe verkauft.
- **h)** _______ ist ein Apparat, _______ man fotografieren oder filmen kann.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 26)</strong></summary>

> *Didaktische Lösungen für **6.2. Relativsätze — Vertiefung & Übungen**:*

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung
- **o)** `—` — Grammatische Ergänzung
- **p)** `—` — Grammatische Ergänzung

#### Übung 4
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 26)**](#workbooks:A2:26) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-27"></a>
## 📄 Seite 27: 6.3. Plusquamperfekt

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 6.3. Plusquamperfekt
> • Er bestellte das Tagesmenü. Vorher hatte er die Speisekarte gelesen.
> • Lena machte eine Pause. Vorher war sie zehn Kilometer gewandert.
> • Max hatte die Speisekarte gelesen, dann bestellte er das Tagesmenü.
> • Lena war zehn Kilometer gewandert, dann machte sie eine Pause.
> • jetzt
> • Zeit
> • Das Plusquamperfekt zeigt, dass eine Aktion vor einer anderen Aktion war.
> • Vorzeitigkeit in der Vergangenheit ➔ hatte / war + Partizip II


### ✍️ Übung 1
*Wählen Sie aus dem Wortkasten: Max, hatte, die, Rechnung, bezahlt, dann, verließ, er, das, Restaurant.*

> 💡 **Beispiel:** Max verließ das Restaurant. (Rechnung bezahlen)

- **a)** Eva bestellte Kaffee. (ein Stück Kuchen essen)
- **b)** Theo zog sich an. (sich duschen)
- **c)** Paul besuchte uns. (mich anrufen)
- **d)** Lena ging zu Bett. (fernsehen)
- **e)** Julia verließ das Haus. (alle Fenster schließen)
- **f)** Robert nahm an der Prüfung teil. (sich sehr gut vorbereiten)
- **g)** Yasmin fuhr in die Stadt. (mit ihrer Schwester telefonieren)
- **h)** Wir setzten uns in ein Café. (zwei Stunden spazieren gehen)

### ✍️ Übung 2
*Wählen Sie aus dem Wortkasten: Ich, schaltete, den, Computer, aus, vorher, hatte, ich, alle, Programme, geschlossen.*

> 💡 **Beispiel:** Ich schaltete den Computer aus. (alle Programme - schließen)

- **a)** Kai ging zu Bett. (die Zähne — sich putzen)
- **b)** Wir gingen ins Hotel. (die Stadt — besichtigen)
- **c)** Ihr wart sehr vorsichtig. (eine schlechte Erfahrung — machen)
- **d)** Endlich fand er seine Brille. (überall — suchen)
- **e)** Wir durften weiterfahren. (unsere Pässe — zeigen)
- **f)** Lena kaufte einen Laptop. (die Preise — vergleichen)
- **g)** Paul zog nach Wien. (zwei Jahre — in Salzburg — leben)
- **h)** Ich löschte das Licht. (noch ein paar Seiten — lesen)
- **i)** Wir fanden endlich eine schöne Wohnung. (fast ein Jahr — suchen)
- **j)** Theo kaufte sich ein neues Sofa. (umziehen)
- **k)** Ich nannte der Taxifahrerin die Adresse. (einsteigen)
- **l)** Man informierte die Polizei. (ein Unfall — passieren)

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 27)</strong></summary>

> *Didaktische Lösungen für **6.3. Plusquamperfekt**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 27)**](#workbooks:A2:27) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-28"></a>
## 📄 Seite 28: 7.1. Nebensätze - damit

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Lektion 7
> • 7.1. Nebensätze - damit
> • Nebensätze  mit damit ➔ Intention, Absicht, Zweck ➔ Frage: wozu?
> • Max schreibt alles auf. Er will nichts vergessen.
> • Max schreibt alles auf, damit er nichts vergisst.
> • Lisa ruft den Ober. Er soll ihr die Speisekarte bringen.
> • Lisa ruft den Ober, damit er ihr die Speisekarte bringt.


### ✍️ Übung 1
*Wählen Sie aus dem Wortkasten: Er, spricht, sehr, laut, damit, alle, Leute, ihn, hören.*

> 💡 **Beispiel:** Er spricht sehr laut. Alle Leute sollen ihn hören.

- **a)** Ich erkläre ihm die Grammatik sehr langsam. Er soll alles verstehen.
- **b)** Max schließt das Fenster. Die Nachbarn sollen die Musik nicht hören.
- **c)** Ich schreibe Carmen eine E — Mail. Sie soll mir mein Buch zurückgeben.
- **d)** Paul gibt seiner Tochter den Wagenschlüssel. Sie soll die Tante vom Bahnhof abholen.
- **e)** Martina schneidet das Obst in kleine Stücke. Ihre Tochter soll mehr davon essen.
- **f)** Die Polizei macht Radarkontrollen. Die Autofahrer sollen nicht so schnell fahren.

### ✍️ Übung 2
*Wählen Sie aus dem Wortkasten: Karl, stellt, sich, auf, einen, Stuhl, damit, er, alles, sehen, kann.*

> 💡 **Beispiel:** Karl stellt sich auf einen Stuhl. Er will alles sehen können.

- **a)** Wir trainieren. Wir wollen fit bleiben.
- **b)** Max braucht einen Termin beim Zahnarzt. Er will nicht so lange warten müssen.
- **c)** Ich schreibe mir den Termin auf. Ich möchte ihn nicht vergessen.
- **d)** *(Carmen)* trinkt viel Zitronentee. Sie will keine Erkältung kriegen.
- **e)** *(Jochen)* spricht mit dem Autohändler. Er will den Wagen billiger bekommen.
- **f)** Claudia macht einen Spanischkurs. Sie will die Sprache besser verstehen können.

### ✍️ Übung 3
*Wählen Sie aus dem Wortkasten: Max, kauft, eine, Brille, damit, er, besser, lesen, kann.*

> 💡 **Beispiel:** Max kauft eine Brille. Er kann dann besser lesen.

- **a)** Petra macht einen Computerkurs. Sie kann dann einen besseren Job bekommen.
- **b)** Jana fährt am Wochenende in die Berge. Sie kann sich dann erholen
- **c)** *(Jürgen)* kauft ein Kochbuch. Er kann dann besser kochen.
- **d)** Martha nimmt eine Schlaftablette. Sie kann dann besser einschlafen.
- **e)** Tom macht einen Online — Kurs. Er kann dann zu Hause lernen.
- **f)** Lisa braucht Ruhe. Sie kann sich dann besser konzentrieren.
- **g)** Robert steht heute früher auf. Er kann dann vor dem Frühstück noch joggen.
- **h)** Tamara fährt lieber mit dem Zug nach München. Sie kann dann unterwegs arbeiten.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 28)</strong></summary>

> *Didaktische Lösungen für **7.1. Nebensätze - damit**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `Carmen` — Grammatische Ergänzung
- **e)** `Jochen` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `Jürgen` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 28)**](#workbooks:A2:28) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-29"></a>
## 📄 Seite 29: 7.2. Unbestimmte Pronomen und Artikel

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 7.2. Unbestimmte Pronomen und Artikel
> • Artikel stehen zusammen mit einem Nomen.
> • Pronomen stehen allein ohne ein Nomen.
> • unbestimmte Artikel / Negation unbestimmte Pronomen / Negation
> • mask. fem. neutr. Plural mask. fem. neutr. Plural
> • Nom. [k]ein [k]eine [k]ein keine Nom. [k]einer [k]eine [k]ein[e]s welche / keine
> • Akk. [k]einen [k]eine  [k]ein keine Akk. [k]einen [k]eine [k]ein[e]s welche / keine
> • Dat. [k]einem [k]einer [k]einem keinen Dat. [k]einem [k]einer [k]einem welchen / keinen
> • Gen. [k]eines [k]einer [k]eines keiner Gen. [k]eines [k]einer [k]eines welcher / keiner
> • keine Endung ➔ Nom. mask. / neutr.
> • ➔ Akk. neutr.      immer mit Endung
> • Ist hier in der Nähe ein Supermarkt? - In der Badstraße ist einer.
> • Hat einer meine Brille gesehen? - Nein keiner hat sie gesehen.
> • Kaufst du Äpfel? - Ja, ich kaufe welche.


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Kann man hier telefonieren?
> • man (nur Nominativ) bezeichnet viele unbestimmte Personen oder die Allgemeinheit.
> • Hast du jemand(en) gesehen? - Ich habe niemand(en) gesehen.
> • jemand benennt eine oder mehrere unbekannte Personen. Die Negation ist niemand.
> • jemand / niemand kann man im Dat. und Akk. deklinieren. ➔ jemand(em) / niemand(en)
> • Im Genitiv muss man jemand / niemand deklinieren. ➔ jemandes / niemandes


### ✍️ Übung 1
*Wählen Sie aus dem Wortkasten: Lebensmittelgeschäfte, Wo, gibt, es, hier, Lebensmittelgeschäfte?, Dort, sind, welche*

> 💡 **Beispiel:** e: Sportplatz    Wo gibt es hier   einen   Sportplatz? - Dort ist   einer  .

- **a)** Tankstelle
- **b)** Supermarkt
- **c)** Museum
- **d)** Schnellimbiss
- **e)** Krankenhaus
- **f)** Bank
- **g)** Reisebüro
- **h)** Bushaltestelle
- **i)** Buchladen
- **j)** Restaurants
- **k)** Werkstatt
- **l)** Taxis

### ✍️ Übung 2
*Ergänzen Sie.*

- **a)** Kann mir mal _______ helfen?
- **b)** Kann _______ die Burg auch im Winter besichtigen?
- **c)** Kann _______ in diesem Geschäft spanischen Wein kaufen?
- **d)** Alle waren still. _______ hat ein Wort gesagt.
- **e)** Ich habe angerufen, aber _______ hat sich gemeldet.
- **f)** Es war so dunkel, dass _______ nichts sehen konnte.
- **g)** Die Party war langweilig. _______ hat getanzt.
- **h)** Weil ich den Weg nicht gekannt habe, habe ich _______ gefragt.
- **i)** _______ ist so klug wie Inge.
- **j)** Hat _______ meine Tasche gesehen?
- **k)** Wenn _______ sich bei Kälte nicht warm anzieht, kann _______ eine Grippe bekommen.
- **l)** Es hat geklopft. Kann mal _______ die Tür aufmachen?
- **m)** Wenn _______ nicht aufpasst, kann _______ viele Fehler machen.
- **n)** Wenn du das allein nicht machen kannst, musst du _______ um Hilfe bitten.
- **o)** Joanas hat so lange gerufen, bis ihn _______ gehört hat.

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 29)</strong></summary>

> *Didaktische Lösungen für **7.2. Unbestimmte Pronomen und Artikel**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung
- **o)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 29)**](#workbooks:A2:29) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-30"></a>
## 📄 Seite 30: 7.3. Nebensätze - Fragesätze

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 7.3. Nebensätze - Fragesätze
> • Wann fährt der Zug ab? - Ich weiß nicht, wann der Zug abfährt.
> • Nebensätze können mit einem Fragewort beginnen.
> • Wann kommt Paul an? - Ich weiß nicht, wann Paul ankommt.
> • In einem Nebensatz steht das Verb am ENDE. (trennbare Verben ➔ zusammen)
> • ich weiß
> • du weißt
> • er / sie / es weiß
> • wir wissen
> • ihr wisst
> • sie / Sie wissen


> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Fragesätze ohne Fragewort  Nebensatz  ➔  Konjunktion ob


### ✍️ Übung 1
*Bilden Sie einen Nebensatz mit Fragewort.*

> 💡 **Beispiel:** Wann    fährt der Zug ab? -  Ich weiß nicht,   wann   der Zug abfährt.

- **a)** _______ ruft in der Firma an? e) _______ stellt die Firma her?
- **b)** _______ steht Theo am Morgen auf?  f) _______ holt dich vom Bahnhof ab?
- **c)** _______ kauft Carmen heute ein? g) _______ lädt dein Kollege ein?
- **d)** _______ fährt nach München mit? h) _______ fängt der Film an?

### ✍️ Übung 2
*Bilden Sie einen Nebensatz mit Fragewort.*

> 💡 **Beispiel:** warum - Tom - so spät - kommen  >  Ich weiß nicht,   warum   Tom so spät gekommen ist.

- **a)** wo — Lena — sein e) wohin — Clara — fahren
- **b)** wann — Max — abreisen  f) wie lange — Film — dauern
- **c)** wem — Jana — helfen g) was — Maria — verlieren
- **d)** wie viel — Stefan — bezahlen h) wen — Hatem — treffen

### ✍️ Übung 3
*Bilden Sie einen Nebensatz mit ob.*

> 💡 **Beispiel:** Kommt Peter heute? -  Ich habe keine Ahnung,   ob   Peter heute kommt.

- **a)** Geht Martina heute Abend ins Kino? g) Fährt Frau Berg nach Rom?
- **b)** Trinkt Thomas Weißwein? h) Nimmt man dich mit?
- **c)** Holt Paul mich vom Flughafen ab?  i) Wird das Wetter morgen besser?
- **d)** Kann man hier kostenlos parken?  j) Kann man diesen Computer noch reparieren?
- **e)** Kauft sich Lisa ein Fahrrad? k) Geht dein Kollege nächste Woche in Urlaub?
- **f)** Spielt Max morgen Fußball?  l) Hat Kai die Nachricht bekommen?

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 30)</strong></summary>

> *Didaktische Lösungen für **7.3. Nebensätze - Fragesätze**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 30)**](#workbooks:A2:30) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-31"></a>
## 📄 Seite 31: 8.1. Demonstrativartikel und Demonstrativpronomen

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • Lektion 8
> • 8.1. Demonstrativartikel und Demonstrativpronomen
> • dieser, diese, dieses
> • Gehört dir dieser Schirm? - Nein, dieser hier.
> • Gefallen dir diese Schuhe? - Nein, aber diese gefallen mir.
> • maskulin feminin neutral Plural
> • Nominativ
> • Akkusativ
> • Dativ
> • dieser diese dieses diese
> • diesen diese dieses diese
> • diesem dieser diesem diesen


### ✍️ Übung 1
*Bilden Sie eine Frage im Präsens.*

> 💡 **Beispiel:** Rechnung - bezahlen  Wann bezahlst du   diese   Rechnung?

- **a)** Reise — buchen
- **b)** Leute — helfen
- **c)** Schloss — besichtigen
- **d)** Geschichte — erzählen
- **e)** Leute — informieren
- **f)** SMS — schreiben
- **g)** Experte — fragen
- **h)** Freundin — anrufen
- **i)** Herr — zuhören
- **j)** Kollege — sprechen
- **k)** Dinge — sich kümmern
- **l)** Frau — sich bedanken

### ✍️ Übung 2
*Bilden Sie eine Frage im Perfekt.*

> 💡 **Beispiel:** lustig - Film - sehen  Hast du   diesen   lustigen Film gesehen?

- **a)** kaputt — Fahrrad — reparieren
- **b)** scharf — Suppe — probieren
- **c)** lang — Brief — schreiben
- **d)** schwer — Rucksack — tragen
- **e)** schwierig — Fragen — beantworten
- **f)** langweilig — Roman — lesen
- **g)** süß — Früchte — essen
- **h)** jung — Journalist — antworten
- **i)** nett — Frau — grüßen
- **j)** bekannt — Experte — vertraut
- **k)** klug — Kollegin — glauben
- **l)** freundlich — Herr — kennen
- **m)** sympathisch — Studentin — einladen
- **n)** arm — Mensch — helfen

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 31)</strong></summary>

> *Didaktische Lösungen für **8.1. Demonstrativartikel und Demonstrativpronomen**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung
- **m)** `—` — Grammatische Ergänzung
- **n)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 31)**](#workbooks:A2:31) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-32"></a>
## 📄 Seite 32: 8.2. Futur I

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 8.2. Futur I
> • werden + Infinitiv Singular Plural
> • ich werde kochen wir werden kochen
> • du wirst kochen ihr werdet kochen
> • er / sie / es wird kochen sie / Sie werden kochen
> • Futur I - nicht nur für die Zukunft
> • Zukunft ➔ Was wird sein? / Was wird passieren?
> • meist: Präsens (+ Zeitinformation): Ich rufe dich (bald / nächste Woche) an. / Unser Gast reist morgen ab.
> • selten: Futur I Ich werde dich anrufen. / Unser Gast wird (morgen) abreisen.
> • Plan / Vorsatz / Versprechen ➔ Was nimmt sich jemand vor? / Was verspricht jemand?
> • Futur I [+ sicher / bestimmt etc.] Ich werde mich sicher gut vorbereiten. / Ich werde dir bestimmt helfen.
> • auch: Präsens + sicher / bestimmt etc. Ich bereite mich sicher gut vor. / Ich helfe dir bestimmt.
> • Vermutung ➔ Was kann vielleicht jetzt oder morgen sein / passieren?
> • Adverb: vielleicht / wahrscheinlich Wahrscheinlich / Vielleicht bleibt Jana heute noch / morgen noch in Wien.
> • Futur I [+ wohl] Jana wird [wohl] heute noch / morgen noch in Wien bleiben.
> • Aufforderung ➔ Was soll jemand machen?
> • Imperativ: Hör endlich zu! / Seid jetzt ruhig!
> • Futur I + wohl Wirst du wohl endlich zuhören! / Werdet ihr jetzt wohl ruhig sein!


### ✍️ Übung 1
*Wählen Sie aus dem Wortkasten: Vorsatz, Versprechen, Antworten, Sie, mit, Futur, sicher.*

> 💡 **Beispiel:** Wann räumst du endlich dein Zimmer auf? - Ich   werde   morgen sicher mein Zimmer   aufräumen  .

- **a)** Wann putzt du endlich die Fenster?
- **b)** Wann entscheidest du dich endlich?
- **c)** Wann reparierst du endlich das Fahrrad?
- **d)** Wann suchst du dir endlich eine neue Wohnung?
- **e)** Wann hörst du endlich mit dem Rauchen auf?
- **f)** Wann rufst du endlich den Handwerker an?
- **g)** Wann kaufst du dir endlich ein Wörterbuch?
- **h)** Wann gibst du mir endlich mein Werkzeug zurück?

### ✍️ Übung 2
*Wählen Sie aus dem Wortkasten: Vermutung, Bilden, Sie, Sätze, mit, Futur, wohl.*

> 💡 **Beispiel:** Besuchst du Thomas bald? - Ich   werde   ihn (Thomas) wohl bald   besuchen  .

- **a)** Rufst du deine Cousine an?
- **b)** Arbeitest du am Wochenende?
- **c)** Fährt deine Kollegin nach Wien?
- **d)** Braucht ihr meine Hilfe?
- **e)** Bringst du Max zum Flughafen?
- **f)** Lädst du Amira zur Party ein?
- **g)** Fliegt ihr nach Kairo?
- **h)** Schafft Hatem die Prüfung?
- **i)** Geht ihr heute Abend ins Kino?
- **j)** Wohnt Sandra jetzt in Köln?
- **k)** Nimmst du an dieser Reise teil?
- **l)** Kaufst du dir dieses E — Bike?

### ✍️ Übung 3
*Wählen Sie aus dem Wortkasten: Aufforderung, Bilden, Sie, Sätze, mit, Futur, wohl.*

> 💡 **Beispiel:** Sag endlich die Wahrheit! - Wirst   du wohl   endlich die Wahrheit   sagen  !

- **a)** Hör endlich auf mich!
- **b)** Sei endlich leise!
- **c)** Lauft schneller!
- **d)** Räum endlich dein Zimmer auf.
- **e)** *(Fahren)* Sie endlich hier weg!
- **f)** Bring endlich den Müll raus!
- **g)** Lasst die Katze in Ruhe!
- **h)** *(Bezahlen)* Sie endlich Ihre Miete!
- **i)** Geht endlich schlafen, Kinder!

<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 32)</strong></summary>

> *Didaktische Lösungen für **8.2. Futur I**:*

#### Übung 1
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung

#### Übung 2
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `—` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `—` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung
- **j)** `—` — Grammatische Ergänzung
- **k)** `—` — Grammatische Ergänzung
- **l)** `—` — Grammatische Ergänzung

#### Übung 3
- **a)** `—` — Grammatische Ergänzung
- **b)** `—` — Grammatische Ergänzung
- **c)** `—` — Grammatische Ergänzung
- **d)** `—` — Grammatische Ergänzung
- **e)** `Fahren` — Grammatische Ergänzung
- **f)** `—` — Grammatische Ergänzung
- **g)** `—` — Grammatische Ergänzung
- **h)** `Bezahlen` — Grammatische Ergänzung
- **i)** `—` — Grammatische Ergänzung

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 32)**](#workbooks:A2:32) mit automatischer Korrektur und Sofort-Feedback.

</details>

---

<a id="seite-33"></a>
## 📄 Seite 33: 8.3. Pauschale Negation

> [!NOTE]
> **📌 Grammatik-Fokus dieser Lektion:**
>
> • 8.3. Pauschale Negation
> • Bei der pauschalen Negation (auch Satznegation) wird die ganze Aussage verneint.
> • Als Negationswort gebraucht man nicht. Die Betonung des Satzes liegt auf nicht.
> • Kaufst du diese Wohnung? - Nein, ich kaufe diese Wohnung nicht.
> • Entscheidet ihr euch heute? - Nein, wir entscheiden uns heute nicht.
> • Oft steht die pauschale Negation weit hinten im Satz, d. h. die meisten Satzglieder stehen vor   nicht  .
> • Folgende Satzglieder stehen in der Regel hinter nicht:
> • 1. Teile, die zum Prädikat gehören
> • Präfixe: Ich rief Paul nicht an.
> • Infinitive: Ich kann dir nicht helfen.
> • Partizip II: Ich habe Max nicht eingeladen.
> • prädikative Nomen: Wir spielen heute nicht Fußball.
> • prädikative Adjektive: Das ist wirklich nicht wichtig.
> • adverbiale Adjektive: Dieser Zug fährt nicht schnell.
> • 2. Ergänzungen (meistens hinter nicht)
> • Präpositionalobjekt: Martin wartet nicht auf uns.
> • 3. Angaben* (meistens hinter nicht)
> • modal: Ich tanze nicht gern.
> • lokal: Die Kinder spielen nicht draußen.
> • Negation bei „noch”  ←➔ „nicht mehr”  / „schon” ←➔ „noch nicht”
> • Schläft Max noch? - Nein, er schläft nicht mehr.
> • Ist die Arbeit schon fertig? - Nein, sie ist noch nicht fertig.
> • * temporale Angaben mit Chronologie (z. B. früher - jetzt - später / gestern - heute - morgen) stehen vor nicht.
> • Ihr arbeitet morgen nicht. / Max geht jetzt nicht nach Hause.
> • temporale Angaben ohne Chronologie (z. B. immer / pünktlich / täglich) stehen hinter nicht.
> • Der Zug kommt nicht pünktlich. / Wir treffen uns nicht täglich.
> • Übung
> • Bilden Sie eine pauschale Negation.
> • Beispiel: Spielst du gerne Karten? -   Nein, ich spiele nicht gerne Karten.
> • a) Nimmt Max an der Konferenz teil?
> • b) Lädst du deinen Kollegen ein?
> • c) Besucht deine Tante dich heute?
> • d) Liest du jetzt die Zeitung?
> • e) Hat Jana sich um diese Stelle beworben?
> • f) Sind die Gäste sofort abgereist?
> • g) Wolltest du dich von ihnen verabschieden?
> • h) Darf man diese Produkte importieren?
> • i) Kommen diese Früchte aus Tunesien?
> • j) Kann Peter die Arbeit rechtzeitig erledigen?
> • k) Konnte der Experte dir das Problem erklären?
> • l) Sind solche Aktionen notwendig?
> • m) Habt ihr die Bücher schon zurückgegeben?
> • n) Dauert die Fahrt noch lange?
> • o) Muss man den Kollegen noch informieren?
> • p) Habt ihr die Zimmer schon reserviert?
> • q) Möchtest du noch bleiben?
> • r) Sind die Arbeiten schon fertig?


<details>
<summary><strong>👉 💡 Musterlösung & Grammatik-Tipps (Seite 33)</strong></summary>

> *Didaktische Lösungen für **8.3. Pauschale Negation**:*

*(Auf dieser Seite befinden sich Einführungstexte, Inhaltsverzeichnisse oder freie Textübungen.)*

---
👉 **Interaktiv üben:** Öffne diese Seite im [**Arbeitsbuch-Studio (Seite 33)**](#workbooks:A2:33) mit automatischer Korrektur und Sofort-Feedback.

</details>

---
